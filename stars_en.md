...
> `#Metadata Extraction` `#Multimedia Processing` `#Java Library` `#eps` `#exif` `#icc` `#iptc` `#java` `#jpeg` `#metadata` `#mp4` `#photography` `#png` `#quicktime` `#tiff` `#video` `#webp` `#xmp` 
> metadata-extractor is a mature Java library for extracting rich metadata from image, video, and audio files. It supports Exif, IPTC, XMP, ICC, and other standards across JPEG, PNG, TIFF, MP4, RAW, and numerous other formats, with deep decoding of manufacturer-specific Makernote data from Canon, Nikon, Sony, and other cameras. Widely adopted for digital asset management, image processing pipelines, and forensic analysis, it offers a clean API requiring just a single line of code to read complete metadata structures. Distributed via Maven Central, it has become the de facto standard for media metadata parsing in the Java ecosystem, featuring zero dependencies, high-performance streaming parsing, and robust error recovery even with corrupted files. Its modular architecture separates format-specific logic, enabling easy maintenance and extension, while the unified metadata abstraction layer shields developers from underlying binary complexity. Actively maintained with tracking of evolving standards from ISO and Adobe, it provides precise audit capabilities increasingly critical for data compliance. The library's design emphasizes memory-efficient processing suitable for high-throughput scenarios, from embedded devices to cloud services. Its commercial-friendly license and extensive documentation lower integration barriers, making it an essential infrastructure component for media-centric applications. The project exemplifies sustainable open-source development through focused vertical expertise, professional governance, and responsive community support, delivering proven value across billions of file processing operations.
...
> `#On-Device AI` `#Android Automation` `#VLM` `#AI Agent` `#agent` `#ai` `#alpine` `#android` `#automation` `#bot` `#claw` `#codex` `#gui` `#linux` `#on-device` `#skills` 
> OpenOmniBot is an on-device AI agent built with native Android Kotlin and Flutter, running locally rather than in the cloud. It leverages vision models to understand and operate Android interfaces like a human, including app control, gestures, and system settings. The project features a full 'understand-decide-execute-reflect' loop with extensible skills, MCP protocol, terminal access, and memory systems, achieving true device-side automation by integrating large models deep into the mobile OS layer.
...
> `#Tauri Desktop App` `#MQTT Debugging Tool` `#IoT Script Preprocessing` `#Cross-Platform Client` 
> Mini MQTT Client is a lightweight cross-platform MQTT desktop debugging tool built with Tauri 2 and Vue 3, supporting Windows, macOS, and Linux. Its core strengths include command template saving, scheduled cyclic publishing, and JavaScript preprocessing scripts for message encryption/decryption and format conversion. It supports MQTT 3.1.1/5.0, TLS/SSL, multi-topic wildcard subscriptions, and QoS 0-2. Targeting embedded engineers and IoT developers, it fills the gap in lightweight desktop MQTT tools with scripting and scheduling capabilities. The Rust backend (rumqttc) and Vue 3 frontend ensure performance and modern development experience. Built-in crypto libraries (AES/SHA/MD5/HMAC) enable bidirectional script processing. Dark/light themes and error logging enhance production usability. MIT licensing lowers enterprise adoption barriers. The Tauri updater mechanism supports continuous delivery, making it a compelling complement to web-based or CLI MQTT clients.
...
> `#AI API Gateway` `#Subscription Quota Distribution` `#Multi-tenant Billing` `#2api` `#antigravity2api` `#cc2api` `#claude` `#claude-code` `#codex` `#crs` `#crs2` `#gemini` 
> Sub2API is an open-source AI API gateway built with Go and Vue, unifying Claude, OpenAI, Gemini subscriptions into a single endpoint. Features multi-account management, intelligent scheduling, token-level billing, concurrency control, and built-in payment support. Enables subscription sharing and cost splitting with native tool compatibility. Stack: Go 1.25.7, Vue 3.4+, PostgreSQL 15+, Redis 7+, Docker. Important: Usage may violate upstream ToS; use at own risk. Compliant use only. Sponsors: OpenModel, ETok.ai.
...
> `#AI Agent Tooling` `#Git Atomic Operations` `#Programmatic VCS` `#History Rewriting` `#ai-agents` `#git` 
> git-surgeon provides surgical Git primitives for autonomous coding agents, eliminating destructive workarounds when agents cannot use interactive commands like `git add -p`. It enables hunk-level staging, committing, and discarding by ID, with line-range precision. Advanced features include commit splitting, history folding, retroactive amendments, and branch-targeted commits—empowering agents to produce granular, well-structured commits without human intervention. Ships with ready-to-use skills for Claude, Codex, and OpenCode.
...
> `#Semantic Code Graph` `#AI Agent Enhancement` `#MCP Server` `#Local-First` 
> CodeGraph is a local semantic code knowledge graph for AI coding agents including Claude Code, Cursor, Codex, and more. It pre-indexes codebase and auto-syncs on changes, cutting token usage by ~16% and tool calls by ~58%. Fully local with zero Node.js dependency, it installs cross-platform via one-liner and wires into agents via MCP server for enhanced code intelligence.
...
> `#Commerce Framework` `#Modular Architecture` `#B2B/DTC` `#commerce` `#e-commerce` `#ecommerce` `#framework` `#javascript` `#medusa` `#nodejs` `#react` `#typescript` 
> Medusa is a modular open-source commerce framework providing customizable core primitives (products, orders, payments, etc.) for building complex B2B, DTC, marketplace, and POS solutions. Its plugin-based architecture enables flexible extension without rebuilding foundational commerce logic.
...
> `#AI Agent Pentesting` `#White-box SAST` `#Autonomous Exploitation` `#DevSecOps Shift-Left` `#penetration-testing` `#pentesting` `#security-audit` `#security-automation` `#security-tools` 
> Shannon is an autonomous white-box AI pentester for web apps and APIs by Keygraph. It analyzes source code to identify attack vectors, then executes real exploits via AI agents with browser automation and CLI tools—only reporting verified vulnerabilities with working PoCs. Shannon Lite (AGPL-3.0) enables local white-box testing, while Shannon Pro delivers enterprise continuous pentesting, SAST, and CI/CD gating to bridge the gap left by annual manual assessments.
...
> `#Edge Computing` `#DNS-over-HTTPS` `#Content Blocking` `#Serverless` `#adblock` `#cloudflare` `#cloudflare-workers` `#deno` `#dns-over-https` `#dns-over-tls` `#doh` `#fastly` `#fastly-compute-at-edge` `#flyio` `#nodejs` `#pihole` `#serverless` `#workers` 
> serverless-dns is a self-hosted, Pi-Hole-esque DNS resolver supporting DoH/DoT protocols with built-in content blocking. Uniquely architected for edge computing, it deploys seamlessly to Cloudflare Workers, Deno Deploy, Fastly Compute@Edge, and Fly.io. Leveraging V8 Isolates and MicroVM runtimes, it achieves sub-2ms server-side processing with free tiers sufficient for household DNS traffic.
...
> `#AI Coding Assistants` `#Multi-Config Management` `#TUI/CLI Dual-Mode` 
> CC-Switch CLI is a Rust-based cross-platform management tool for AI coding assistants, featuring dual TUI/CLI modes. It unifies configuration switching, MCP servers, skills, prompts, and proxy routing across Claude Code, Codex, Gemini and more, with WebDAV sync and usage analytics to streamline multi-tool developer workflows.
...
> `#AI Coding Assistant` `#Multi-Model Integration` `#Tauri Desktop App` `#ai-tools` `#claude-code` `#codex` `#desktop-app` `#hermes` `#hermes-agent` `#mcp` `#minimax` `#omo` `#open-source` `#openclaw` `#openclaw-ui` `#opencode` `#provider-management` `#rust` `#skills` `#skills-management` `#tauri` `#typescript` `#wsl-support` 
...
> `#WireGuard Mesh` `#Zero Trust Networking` `#NAT Traversal` `#2fa` `#oauth` `#sso` `#tailscale` `#vpn` `#wireguard` 
> Tailscale is an open-source networking tool built on WireGuard that simplifies secure private network creation. This core repository contains the tailscaled daemon and CLI, supporting Linux, Windows, macOS and BSD. Written in Go, it features zero-config setup, NAT traversal, and granular ACLs with 2FA support, widely used for remote work, cloud resource interconnectivity, and edge device management.
...
> `#Log Analytics Engine` `#Zero-dependency Binary` `#Columnar Compression Index` `#analytics` `#database` `#devops` `#golang` `#logging` `#splunk` 
> LynxDB is a zero-dependency, single-binary log analytics engine. Its Lynx Flow pipeline language lets you stream, parse and aggregate terabytes of logs from stdin or a persistent server with full-text search, columnar compression and Roaring bitmap indexes in seconds.
...
> `#RSS Aggregation` `#AI Summarization` `#Event Clustering` `#Daily Digest` `#ai-tools` `#rss` `#rss-aggregator` `#rss-feed` `#summary` 
> Infinitum is an RSS-based news aggregation workbench that concurrently fetches, enriches, summarizes and clusters articles via AI, generating daily digests while letting users filter noise, monitor tasks and publish curated RSS feeds.
...
> `#AutoLISP Automation` `#Dual-backend MCP` 
> MCP server for AutoCAD LT 2024+ offering dual backends—File IPC for live AutoLISP and ezdxf for headless DXF—exposing 8 unified tools (drawing, entity, layer, block, annotation, P&ID, view, system) with focus-free dispatch, full undo/redo, UTF-8 fallback and cross-platform DXF generation.
...
> `#AI usage tracking` `#CLI telemetry` `#Web dashboard` `#Privacy-first` 
> Token Arena is an open-source AI usage tracker that auto-collects token consumption from local AI coding CLIs, delivering multi-dimensional dashboards, leaderboards, OAuth2 login, privacy-first hashing and one-click Docker deployment.
...
> `#Self-hosted Agent Orchestration` `#Cross-device Sync` `#Zero-telemetry Privacy` `#ade` `#agents` `#claude-code` `#codex` `#copilot` `#developer-tools` `#gemini` `#mobile` `#opencode` `#orchestration` `#pi` 
> Paseo is a self-hosted, cross-device orchestration platform that unifies Claude Code, Codex and OpenCode agents. Run parallel tasks on your own machines, control them via voice or CLI, and switch seamlessly between phone, desktop and web with zero telemetry.
...
> `#HTML-native Design` `#Claude Code Skill` 
> One prompt in Claude Code yields hi-fi prototypes, slides, motion graphics & infographics; 20 design philosophies + 5D review built-in, MP4/PPTX/PDF export—no Figma or AE needed.
...
> [!info]
> ⭐ 2,896 · Rust · 2026-06-08  
> [GitHub](https://github.com/graykode/abtop)  
> `#AI Agent Monitor` `#Terminal TUI` `#Context Window` `#Rate Limits` `#ai-agents` `#ai-coding-agent` `#btop` `#claude-code` `#cli` `#codex` `#developer-tools` `#htop` `#monitor` `#ratatui` `#rust` `#terminal` `#tui` 
> abtop is a real-time TUI monitor for AI coding agents—like htop for Claude Code & Codex CLI. Track token burn, context-window %, rate limits, child processes and open ports across sessions without API keys; tmux-aware pane jumping and 12 themes included.
...
> `#AI Agent Office` `#Zero-dependency CLI` `#Office Automation` `#agent` `#ai` `#cli` `#office` `#openclaw` `#skills` 
> OfficeCLI is the world’s first CLI Office suite purpose-built for AI agents—single binary, zero dependencies, no Office install—giving agents full read/write/automation control over Word, Excel, and PowerPoint in one line.
...
> `#PDFium Engine` `#Portable Zero-Deps` `#GPLv3 OSS` `#Windows Native` `#dotnet` `#gplv3` `#opensource` `#pdf` `#pdf-editor` `#portable` `#windows` `#wpf` 
> Ultra-light Windows PDF editor in a 6 MB portable EXE; merge, split, inline text edit, annotate, sign, password unlock—no install, no account, no telemetry. GPLv3, Adobe-free freedom.
...
> `#MCP Protocol` `#Code Search` `#kimi` `#kimi-cli` `#kimi-code` `#kimi-search` 
> Turns the previously Kimi-CLI-only search engine into a universal MCP server, offering single & batch keyword search plus page fetch, already verified with Claude Code and ready for any MCP client.
...
> `#Obsidian Skills` `#AI Agent` `#Vault Automation` `#agents` `#agentskills` `#bases` `#claude` `#clawdbot` `#cli` `#codex` `#defuddle` `#hermes` `#jsoncanvas` `#markdown` `#md` `#obsidian` `#openclaw` `#opencode` `#skills` 
> A set of Agent Skills for Obsidian that conform to the Agent Skills spec, enabling AI agents like Claude Code and Codex CLI to create/edit Markdown, Bases, JSON Canvas and control the Obsidian CLI for automated vault management and plugin development.
...
> `#Claude Plugin` `#Token Compression` `#Minimal Output` `#ai` `#anthropic` `#caveman` `#claude` `#claude-code` `#llm` `#meme` `#prompt-engineering` `#skill` `#tokens` 
> Claude Code plugin caveman compresses LLM responses to caveman-style minimalism, slashing tokens by ~75% while retaining full technical fidelity. Features Wenyan mode, input compression, one-line reviews, and one-command install for faster, cheaper coding.
...
> `#AI Coding Agent` `#Open-source CLI` 
> OpenCode is the first fully open-source AI coding agent, shipping CLI, desktop and SDKs. One-line install via npm/brew/scoop, runs locally and embeds into any IDE to automate generation, refactoring and debugging end-to-end.
...
> `#Go Auth Library` `#OAuth2 Plugin` `#SAML SSO` 
> A drop-in, self-hosted user authentication library for Go that bundles email/password, OAuth (Google, Facebook, Twitter) and enterprise SAML SSO. Pluggable OAuth2 interface lets you add any provider in minutes; built-in rate limiting, secure sessions, and password hashing included. Works with SQLite or PostgreSQL.
...
> `#SSH tunneling` `#Zero-config ingress` `#expose` `#internet` `#localhost` `#proxy` `#reverse-proxy` `#tunnel` 
> Tunnl.gg is a minimal SSH tunneling service that exposes your localhost:8080 to the internet with one command, auto-provisioning memorable subdomains and Let's Encrypt certificates. It ships with multi-layer rate-limiting, anti-abuse, phishing interstitial, WebSocket support, and requires zero client configuration or authentication.
...
> `#Wails Desktop App` `#Static Site Generator` `#Monaco Editor` `#One-Click Deploy` `#ai-blog` `#ai-blog-generator` `#blog` `#blog-editor` `#blog-engine` `#blog-writing` `#go` `#gridea` `#gridea-pro` `#hexo` `#hexo-alternative` `#hugo` `#hugo-alternative` `#static-blog` `#static-blog-engine` `#static-blog-generator` `#static-site` `#wails-app` 
> Gridea Pro is a next-gen desktop static blog client rebuilt with Go+Wails+Vue3. It offers zero-config setup, Monaco editor, AI assistance, 9 built-in themes and one-click deploy to 6 platforms including GitHub Pages & Vercel, letting writers focus on content.
...
> `#LLM Coding Rules` `#Claude Code Plugin` 
> A distilled CLAUDE.md that turns Andrej Karpathy’s insights on LLM coding pitfalls into four actionable rules—Think First, Simplicity, Surgical Edits, Goal-Driven—installable as a Claude Code plugin to curb hallucinations and bloat.
...
> `#Zero-intrusion Agent` `#Local Discovery` `#Bytecode Enhancement` 
> Zero-intrusion local microservice discovery agent: one JVM flag lets local services find each other instantly while transparently falling back to remote registries (Nacos/Eureka/Consul). Supports Feign, RestTemplate, Gateway, WebClient without code, config or POM changes—restoring full debug chains and eliminating environment contention.
...
> `#Terminal Animation` `#Git Visualization` `#cli` `#cli-tool` `#code-animation` `#commit-history` `#developer-tools` `#git` `#git-history` `#git-visualization` `#productivity` `#ratatui` `#rust` `#screensaver` `#syntax-highlighting` `#terminal` `#terminal-app` `#terminal-based` `#terminal-screensaver` `#tree-sitter` `#tui` `#visualization` 
> gitlogue turns Git history into a cinematic terminal experience: animated typing, syntax highlighting and file-tree transitions let you replay any commit range with themes, speed control and filters—perfect for screensavers, teaching, demos or content creation.
...
> `#VS Code Extension` `#Git Visualization` `#Code Review` `#Branch Filtering` 
> Enhanced VS Code extension that renders Git history as an interactive graph panel, adding multi-select filtering, unrelated-history merge, persistent code reviews, jump-to-HEAD and 20+ refinements for large-repo collaboration.
...
> `#Tauri2+Rust Native Terminal` `#Pure Rust SSH sans OpenSSL` `#agent` `#ai-terminal` `#bring-your-own-key` `#devtools` `#filemanager` `#local-first` `#mcp` `#port-forwarding` `#rag` `#remote-development` `#russh` `#rust` `#sftp` `#ssh-client` `#tauri` `#terminal` `#terminal-emulator` `#tool-calling` `#wslg` `#xterm-js` 
> OxideTerm is a zero-dependency native terminal built with Tauri2+Rust, bundling SSH, SFTP, remote IDE, AI agent and file manager. Features smart reconnect, MCP, RAG, plugins, 30+ themes and 11 languages—no OpenSSL or Electron.
...
> `#WeChat iLink Protocol` `#Cross-platform Messaging` `#App Marketplace Extension` `#Zero-config Self-host` `#ai-chatbot` `#ai-tools` `#app-marketplace` `#bot-management` `#chatbot` `#dingtalk` `#discord` `#golang` `#ilink` `#lark` `#notion` `#react` `#self-hosted` `#slack` `#webhook` `#wechat` `#wechat-bot` 
> OpeniLink Hub is the first open-source, self-hosted Bot management platform built on WeChat ClawBot’s iLink protocol. Bind multiple WeChat accounts via QR scan, install 20+ cross-platform apps in one click to bridge Lark, Slack, Discord, GitHub, Notion, etc., and extend AI capabilities. Features 7-language SDKs, WebSocket/Webhook/AI triple routing, 24 h session renewal, and full traceability—runs out of the box.
...
> `#WeChat Webhook` `#HTTP Gateway` `#Lightweight Notify` 
> A lightweight notification gateway that leverages WeChat’s official APIs to deliver server alerts or script results to your WeChat instantly via simple HTTP webhooks, with Docker deployment and a management panel.
...
> `#cron scheduler` `#Windows task automation` `#Wails cross-platform` `#IPC remote control` `#cron` `#cronjob-scheduler` `#hotkeys` `#scheduler` `#utilities` `#wails3` `#windows-app` 
> Single-binary, low-overhead Windows task scheduler driven by Unix-like cron expressions. Offers concurrency control, failure auto-disable, logs, tray mode and IPC CLI, replacing the bulky built-in Task Scheduler.
...
> `#HID++ Protocol` `#Local Remapping` `#Cross-Platform Hooks` `#automation` `#controler` `#free` `#linux` `#linux-app` `#logi` `#logitech` `#logitech-options` `#macos` `#macos-app` `#mouse` `#mouse-remapping` `#mx-master` `#mx-master-3s` `#mx-master-4` `#open-source` `#productivity` `#python` 
> Mouser is a zero-dependency, zero-telemetry local remapper for Logitech HID++ mice, drop-in replacement for Options+. Offers full HID++ protocol, cross-platform native hooks, per-app profiles, real-time DPI & Smart-Shift control, with MX Master family as first-class citizen.
...
> `#Natural Language GUI` `#Extension-Free Script` `#AI Copilot` `#Accessibility` `#agent` `#ai` `#ai-agents` `#browser-automation` `#javascript` `#mcp` `#typescript` `#web` 
> Alibaba’s ultra-light in-page GUI agent; one-line script embeds natural-language web control without extensions, backends or vision models, turning any site into an AI copilot, smart form filler or accessibility assistant.
...
> `#Rust Proxy` `#Token Compression` `#agentic-coding` `#ai-coding` `#anthropic` `#claude-code` `#cli` `#command-line-tool` `#cost-reduction` `#developer-tools` `#llm` `#open-source` `#productivity` `#rust` `#token-optimization` 
> RTK is a zero-dependency Rust CLI proxy that cuts LLM token usage by 60-90% on everyday dev commands; single binary, <10 ms overhead, 100+ supported commands.
...
> [!info]
> ⭐ 16,047 · Rust · 2026-06-15  
> [GitHub](https://github.com/louis-e/arnis) · [Website](https://arnismc.com/)  
> `#Geo-to-MC` `#World Generator` `#Cross-Platform OSS` `#maps` `#minecraft` `#openstreetmap` `#osm` `#overpass-api` `#rust` `#tauri` 
> Arnis turns any real-world location into high-fidelity Minecraft Java/Bedrock worlds using OpenStreetMap & elevation data. GUI/CLI, open-source, cross-platform, with customizable scale, interiors and spawn.
...
> [!info]
> ⭐ 16,830 · TypeScript · 2026-06-15  
> [GitHub](https://github.com/pascalorg/editor) · [Website](https://editor.pascal.app)  
> `#React Three Fiber` `#WebGPU Rendering` `#Architectural Visualization` `#Turborepo Architecture` 
> Web-first 3D building editor powered by React Three Fiber & WebGPU. Turborepo splits logic into @pascal-app/core (scene state, node schemas), viewer (WebGPU rendering), and editor (interactive tools). Features node hierarchy, undo/redo, IndexedDB persistence, and one-click sharing of architectural projects.
...
> [!info]
> ⭐ 5,387 · TypeScript · 2026-03-25  
> [GitHub](https://github.com/shuding/cobe) · [Website](https://cobe.vercel.app)  
> `#WebGL Globe` `#Zero-Dependency` `#CSS Anchor` `#Data Viz` `#globe` `#webgl` 
> A 5 KB zero-dependency WebGL globe that binds any DOM element as markers, animates with CSS, and renders high-perf arcs & points for interactive data globes.
...
> `#AI Coding Bridge` `#No Public IP` `#Multi-IM` 
> cc-connect bridges local AI coding agents (Claude Code, Cursor, Gemini CLI, Codex) to messaging platforms like Slack, Telegram, Feishu, etc. Chat with your dev AI anywhere—no public IP needed for most channels. One-command setup, multi-model ready.
...
> `#format-string exploit` `#Turing-complete` `#IOCCC` 
> Complete tic-tac-toe in a single printf call: exploits format-string side effects for memory read/write, branching and looping; scanf handles user input. Winner of IOCCC 2020, showcasing printf's Turing-completeness.
...
> `#AI Agents` `#Local Coworking` `#Task Automation` `#acp` `#agent-team` `#ai` `#ai-agent` `#chat` `#chatbot` `#claude-code` `#clawdbot` `#codex` `#cowork` `#gemini` `#gemini-cli` `#hermes` `#llm` `#nano-banana` `#office` `#openclaw` `#opencode` `#skills` `#webui` 
...
> `#Model Context Protocol` `#LeetCode API` `#Programming Problems` `#Automation` `#ai` `#algorithm` `#lc` `#leetcode` `#llm` `#mcp` `#mcp-server` 
> A Model Context Protocol (MCP) server integrating LeetCode APIs for both leetcode.com and leetcode.cn platforms. Provides access to problem descriptions, editorial solutions, user submissions, contest data, and with authentication, private notes and progress tracking. Enables AI assistants to interact with programming challenges programmatically.
...
> `#Multi-Agent Collaboration` `#AI CLI Orchestration` `#Tauri Desktop App` `#Rust + Vue3` `#agent` `#ai` `#automation` `#chatgpt` `#claude` `#claude-code` `#codex` `#codex-cli` `#desktop-app` `#developer` `#developer-tools` `#gemini` `#multi-agent` `#openai` `#openclaw` `#opencode` `#orchestration` `#productivity` `#qwen-coder` `#workflows` 
...
> `#Financial Agent` `#AKShare` `#Plan-Execute` `#LLM Trading` `#agent` `#fiancial` `#financial-research` `#llm` 
> OpenFR is a lightweight AI-powered financial research agent built on LLMs, integrating AKShare for automated stock, fund, futures, and macro data analysis. It uses a Plan-Execute architecture to first plan research steps then execute tools, supporting 15+ LLM providers with multi-source failover, parallel execution, intelligent caching, and a Rich CLI interface.
...
> `#WeChat Official Account` `#Obsidian Plugin` `#HTML Conversion` `#Content Publishing` `#obsidian` `#obsidian-plugin` 
> An Obsidian plugin that converts documents to WeChat Official Account clipboard-ready HTML format. Supports full Markdown content, local and network images with 18 built-in article layout styles. No external services or API keys required—just execute a command to copy and paste for publishing.
...
> `#Tauri` `#SSH Client` `#Cross-platform` `#Jump Server` 
> ZenSSH is a cross-platform SSH client built with Tauri, supporting SSH connections, SFTP file transfer, and jump server configuration. Designed lightweight for performance and stability, featuring Gist config sync for daily SSH use by developers and O&M engineers. Licensed under MIT + Commons Clause, commercial use prohibited.
...
> `#Multi-Agent Framework` `#AI Coding Tools` `#Workspace Isolation` `#Team Collaboration` `#agentic-coding` `#ai-workflow` `#claudecode` `#codex` `#harness` 
...
> `#AI Agent Framework` `#Cross-Platform Toolchain` `#Security Scanning` `#ai-agents` `#anthropic` `#claude` `#claude-code` `#developer-tools` `#llm` `#mcp` `#productivity` 
> ECC is a performance optimization operating system for multi-AI agent harnesses including Claude Code, Codex, and Cursor. It enables production-ready agent workflows through skill orchestration, instinctive reactions, memory optimization, security scanning, and research-first development paradigms with cross-platform reusability.
...
> `#MCP Protocol` `#JetBrains Plugin` `#AI Debugging Assistant` `#Programmatic Debugger` 
> A JetBrains IDE plugin exposing an MCP server that grants AI coding assistants full programmatic control over the debugger. Features 22 tools including breakpoints, conditional breakpoints, step execution, variable inspection/modification, expression evaluation, and stack trace navigation across major IDEs like IntelliJ IDEA, PyCharm, and WebStorm.
...
> `#Tauri 2` `#MQTT Client` `#IoT Debugging` `#Message Encryption` 
> A lightweight MQTT desktop client built with Tauri 2 and Vue 3, supporting MQTT 3.1.1/5.0 protocols with TLS/SSL connections. Core features include command template management, scheduled/cyclic publishing, JavaScript pre-processing scripts (AES/SHA/MD5/HMAC encryption), dark/light themes, and customizable data storage paths for IoT device debugging and message queue testing.
> <sub>一个开源的mqtt 桌面客户端工具。支持保存命令、定时循环发送和预处理脚本</sub>
...
> `#Claude Code Skill` `#gh CLI Tool` `#AI Knowledge Base` `#Local Repository Management` 
> A local GitHub repository knowledge base manager built with gh CLI and Claude Code. It enables developers to clone AI-related projects locally, leverage Claude's analytical capabilities for tech stack recommendations, architecture research, and product solution design. Ideal for building AI tools and learning research.
...
> `#Embedded Database` `#Spring Integration Testing` `#Database Isolation` `#Testcontainers` `#embedded-database` `#embedded-postgresql` `#flyway` `#h2-database` `#liquibase` `#mariadb` `#mssql` `#mysql` `#postgres` `#postgresql` `#spring-boot` `#spring-framework` `#spring-test` `#testcontainers` 
> A library creating isolated embedded databases for Spring integration tests, supporting PostgreSQL, MySQL, MariaDB, MSSQL, H2 and more. It uses template databases for fast cloning and refresh, providing test isolation without @DirtiesContext, with Flyway/Liquibase migration caching to significantly reduce initialization time in large projects.
...
> `#MCP Service Management` `#Reverse Proxy` `#Service Grouping` `#Anthropic Skills` `#ai-agent` `#mcp` `#mcp-convert-to-skill` `#mcp-manager` `#mcp-proxy` `#modelcontextprotocol` 
> One MCP is a centralized reverse-proxy platform for Model Context Protocol services. It enables installing, configuring, and monitoring MCP servers (stdio/SSE/streamable HTTP), grouping multiple services, and exporting as Anthropic Skills for Claude Code. Features real-time analytics, OAuth login (GitHub/Google), RBAC, built with Go + React, Docker-ready.
...
> `#Language Server Protocol` `#AI Coding Assistant` `#Semantic Code Analysis` `#Intelligent Code Navigation` 
> An IntelliSense skill package for AI coding agents built on the Language Server Agent Protocol (LSAP). It provides compiler-accurate code comprehension including semantic navigation, cross-file dependency analysis, and type inference. Native support for Python and Rust enables precise code structure awareness and context understanding, replacing traditional text-based search.
...
> `#AI Investment Assistant` `#Rule-Based Trading` `#Explainable AI` `#Trend Monitoring` 
> InvestAI is an AI-powered investment monitoring assistant that continuously analyzes stocks based on predefined trading rules (e.g., Turtle Trading Rules), identifying buy signals, risk triggers, and trend structure changes. It doesn't predict prices or execute trades—instead, it presents explainable rule-based analysis to help investors stay rational and maintain disciplined decision-making.
...
> `#VS Code Extension` `#AI Chat History` `#Local-First Storage` `#Knowledge Base` `#ai` `#ai-agents` `#cursor` `#visual-studio-code` 
> WayLog is a local-first VS Code extension that preserves AI chat history across multiple assistants including Cursor, Claude Code, Copilot, and Lingma. It aggregates conversations into Git-friendly local storage with background auto-save and selective saving, ensuring developers never lose AI interaction context.
...
> `#Daemonless Build` `#Java Containerization` `#Maven Plugin` `#Gradle Plugin` `#containers` `#docker` `#docker-registry` `#gradle` `#gradle-plugin` `#java` `#jib` `#kubernetes` `#maven` `#maven-plugin` `#microservices` `#oci` 
> Jib is Google's Java container image builder that creates optimized Docker and OCI images without a Docker daemon. Available as Maven/Gradle plugins, it separates dependencies from application code, enabling reproducible builds and significantly faster deployment.
...
> `#MCP Protocol` `#AI Task Management` `#Intelligent Automation` `#Flask Backend` `#ai-coding-tools` `#cursor` `#kiro` `#todo4ai` `#trae` `#windsurf` 
> A purpose-built task management system for AI assistants, featuring native MCP protocol integration with Claude, GPT, and other AI agents. Enables intelligent project management, automated task tracking, and real-time collaboration with AI-powered analytics. Built with React, Flask, and Docker.
...
> `#Spring Boot Auto-configuration` `#SQL Logging` `#Connection Pool Monitoring` `#DataSource Proxy` `#connection-pool` `#datasource` `#datasource-proxy` `#flexy-pool` `#jdbc` `#micrometer` `#p6spy` `#spring-boot` `#spring-boot-starters` `#spring-cloud-sleuth` `#sql-queries` 
> Spring Boot auto-configuration library that decorates DataSource with P6Spy for SQL logging, Datasource Proxy for query interception, and FlexyPool for connection pool metrics. Provides unified configuration via @ConfigurationProperties, supports flexible proxy chains, and enables out-of-the-box data source enhancement for applications requiring SQL monitoring and connection pool optimization.
...
> [!info]
> ⭐ 25,398 · Python · 2026-06-15  
> [GitHub](https://github.com/oraios/serena) · [Website](https://oraios.github.io/serena)  
> `#Semantic Code Retrieval` `#Model Context Protocol` `#LLM Coding Agent` `#Symbol-level Editing` `#agent` `#ai` `#ai-coding` `#claude` `#claude-code` `#codex` `#ide` `#jetbrains` `#language-server` `#mcp-server` `#programming` `#vibe-coding` 
...
> `#Go` `#IoT Hub` `#Device Shadow` `#Private Deployment` `#device-shadow` `#go` `#iot` `#iothub` `#job` `#lightweight` `#mqtt` `#mysql` `#shadow` `#sqlite` `#vue3` 
> tio is a lightweight IoT Hub implementation in Go, designed for private deployment. Core features include Thing management, Device Shadow, Direct Method, and Job scheduling. Built-in MQTT Broker with EMQX support, following cloud-native IoT patterns from AWS IoT and Azure IoT Hub. Production-ready.
...
> `#MQTT v5` `#Go` `#IoT` `#Message Broker` `#golang` `#golang-library` `#golang-package` `#iot` `#mqtt` `#mqtt-broker` `#mqtt-server` `#mqtt-smarthome` `#mqtt3` `#mqtt5` `#mqttv5` `#tcp` `#websocket` 
> Mochi-MQTT is a fully compliant, embeddable high-performance Go MQTT v5 broker with v3.1.1 compatibility. Features TCP/WebSocket with SSL/TLS, built-in Redis/Badger/Pebble/Bolt persistence, flexible Hook extension system, and rule-based auth ACL. Supports QoS0/1/2, shared subscriptions, topic aliases, and message expiry. Ideal for IoT, smart home and pub/sub applications.
...
> `#Git-based CMS` `#Static Sites` `#Content Management` `#Open Source` `#11ty` `#astro` `#cms` `#docusaurus` `#eleventy` `#gatsby` `#hugo` `#jekyll` `#nextjs` `#ssg` `#static-site` `#static-site-generator` `#vitepress` `#vue` `#vuejs` `#vuejs3` `#vuepress` 
> Pages CMS is an open-source content management system for GitHub repositories, optimized for static site generators like Jekyll, Hugo, Next.js, and Astro. It enables visual content and media management directly within GitHub repos without complex configuration. Supports both cloud-hosted and self-hosted deployments, leveraging GitHub App integration for version control and collaborative workflows.
...
> `#Web Aggregation` `#Tab Management` `#Vue3` `#Electron Desktop App` 
> A frontend web tool displaying multiple websites in a configurable grid layout. Supports multiple layout scenarios (work, study, entertainment) with customizable rows/columns. Features CSS selector-based fullscreen mode to isolate specific elements like video players or article content. Offers Chrome extension and Electron desktop client with layout sharing and discovery capabilities.
...
> `#Docker Offline Import` `#Image Tar Export` `#Zero Dependency` `#Mirror Acceleration` `#docker` `#docker-image-tar` `#image-puller` `#multi-arch` `#tar` 
...
> `#BeiDou Grid Code` `#Spatial Index` `#Geo-encoding` `#GIS` 
> Java implementation of BeiDou Grid Code encoder/decoder based on GB/T 39409-2020 national standard. Supports 2D/3D coordinate encoding/decoding with altitude info, and spatial query for intersecting grid codes with polygons, lines, and points. Ideal for geofencing and GIS applications.
...
> `#Java Framework` `#Plugin Architecture` `#Annotation-Driven` `#Web Development` `#ai-agent` `#ai-development` `#framework` `#java` `#java-ai` `#java-cli` `#java-frameworks` `#java-libraries` `#lightweight-framework` `#mcp` `#mcp-server` `#modular-design` `#netty` `#sse` `#tinystruct` `#tomcat` `#undertow` `#web-frameworks` 
> tinystruct is a lightweight, modular Java framework supporting both Web and CLI development. Its plugin-based architecture uses @Action annotations to define HTTP endpoints or CLI commands with automatic routing. Designed for AI integration, it enables building microservices, RESTful APIs, and CLI tools with clean, efficient code. Version 1.7.18.
...
> `#Everything Search` `#AI Natural Language Search` `#Electron Desktop App` `#Vue3 Frontend` 
> A desktop client built on Everything's ultra-fast search engine, using OpenAI GPT to convert natural language queries into precise search syntax. Features one-click connection, multi-strategy process management, glassmorphism UI, multi-dimensional sorting, and supports 10 languages for efficient file retrieval on Windows.
...
> `#FastAPI` `#Data Exfiltration` `#Backdoor Vulnerabilities` 
> A FastAPI-based real-time customer-service bot for Xianyu that listens via WebSocket and auto-replies, yet embeds hard-coded keys, backdoors and data-exfil channels able to steal cookies, chat logs and email verification codes—high-risk usage.
...
> `#MCP Protocol` `#Database AI Agent` `#NL2SQL Framework` `#agent` `#agents` `#ai` `#bigquery` `#clickhouse` `#cockroachdb` `#database` `#elasticsearch` `#firestore` `#genai` `#llm` `#mcp` `#mongodb` `#mysql` `#oracle` `#postgresql` `#redis` `#server` `#spanner` `#tidb` 
> MCP Toolbox for Databases is Google’s open-source MCP server that gives AI agents and IDEs instant, secure access to enterprise databases. Use prebuilt tools for zero-code data exploration or its framework to craft production-grade, NL2SQL-enabled custom tools, with SDKs in Go, Python, JS and Java.
...
> `#Human Verification` `#Java Backend` `#CAPTCHA` `#Bot Protection` 
> Java backend implementation of CAPTCHA human verification system, providing challenge creation, validation and token redemption. Supports custom storage implementation, configurable challenge parameters (count/size/difficulty) and token expiration, suitable for web application bot protection.
...
> [!info]
> ⭐ 6,844 · JavaScript · 2026-06-11  
> [GitHub](https://github.com/tiagozip/cap) · [Website](https://trycap.dev)  
> `#Proof-of-Work` `#Privacy-First` `#Self-Hosted` `#Web Security` `#anti-abuse` `#anti-bot` `#anti-scraper` `#antispam` `#captcha` `#defense` `#javascript` `#proof-of-work` 
...
> `#SHA-256 PoW` `#Cloudflare Workers` `#Edge Computing CAPTCHA` `#Durable Objects` `#captcha` `#cloudflare` `#cloudflare-workers` `#ddos-protection` `#proof-of-work` `#recapcha` 
> A next-generation CAPTCHA service built on Cloudflare Workers, utilizing SHA-256 Proof of Work algorithm for robust bot protection. Deployed across 250+ edge locations globally with sub-100ms response times. Implements Durable Objects for state consistency, provides RESTful API, supports 5-minute integration, and prioritizes user privacy with zero tracking or data collection.
...
> [!info]
> ⭐ 5,157 · JavaScript · 2026-06-11  
> [GitHub](https://github.com/brianpetro/obsidian-smart-connections) · [Website](https://smartconnections.app)  
> `#AI Embeddings` `#Obsidian Plugin` `#Local-First` `#Knowledge Management` `#chatgpt` `#claude` `#embeddings` `#gemini` `#obsidian` `#obsidian-md` `#obsidian-plugin` `#related-items` `#semantic-search` `#vectors` 
...
> `#OpenGL` `#Computer Graphics` `#Graphics Programming` `#Chinese Translation` `#learnopengl` `#translation` 
> A Chinese translation project of the learnopengl.com tutorial series, built with MkDocs static site generator. Covers essential graphics programming topics including OpenGL fundamentals, advanced lighting, shadows, and PBR rendering with complete code examples. Continuously updated with community contributions.
...
> `#3D Tiles` `#Cesium` `#Vulkan` `#Geospatial Visualization` `#cesium` `#cesium-ion` `#gis` `#terrain-visualization` `#vsg` `#vulkan` 
> vsgCs is a Cesium client for Vulkan Scene Graph enabling streaming and rendering of 3D Tiles and Cesium ion geospatial assets. Built on Cesium Native, it supports glTF models, OSM buildings, and global terrain with multi-tileset management and image overlays, ideal for smart city, digital twin, and GIS applications.
...
> `#Electron` `#Vue.js` `#Snippet Manager` `#Developer Tools` `#api-client` `#code-snippets` `#developer-tools` `#electron` `#http-client` `#knowledge-base` `#linux` `#local-first` `#macos` `#markdown` `#note-taking` `#notes` `#productivity` `#snippets` `#snippets-manager` `#vue` `#windows` 
...
> `#Cloud-Native Gateway` `#Wasm Plugins` `#MCP Hosting` `#AI Traffic Governance` `#ai-gateway` `#ai-native` `#api-gateway` `#cloud-native` `#envoy` 
> Higress is a cloud-native AI gateway built on Istio & Envoy, extended via Wasm plugins. It unifies LLM APIs and MCP servers, supports all major model providers, and converts OpenAPI to MCP instantly. Powers Alibaba’s Tongyi & PAI with 99.99% SLA.
...
> `#Rust` `#CLOC Tool` `#Serverless` `#Async Streaming` `#cli` `#cloc` `#cloudflare-workers` `#code` `#code-statistics` `#github` `#gitlab` `#rust` `#wasi` 
> Hyper-fast Rust-based CLOC tool for remote repositories featuring async streaming and in-memory decompression with <12MB memory footprint. Supports 7+ platforms including GitHub/GitLab/Bitbucket, detects 150+ languages, offers real-time progress tracking and multi-format output (JSON/CSV/XML/YAML/TOML), deployable to Cloudflare Workers.
...
> `#Go Streaming Server` `#Multi-Protocol Transcoding` `#RTMP/RTSP/HLS` `#GB28181 SIP` `#aac` `#flash` `#flv` `#golang` `#golang-rtmp` `#h264-avc` `#h265-hevc` `#hls` `#http-ts` `#https-flv` `#lal` `#live-media-server` `#livestream` `#m3u8` `#mpegts` `#rtcp` `#rtmp` `#rtp` `#rtsp` `#video` 
> LAL is a Go-based audio/video live streaming server supporting RTMP, RTSP, HLS, HTTP[S]/WebSocket-FLV/TS, GB28181 protocols and H264/H265/AAC/G711/OPUS codecs. Features relay, cluster, recording, HTTP API/Notify/WebUI with minimal dependencies and flexible deployment.
...
> `#Puppeteer Automation` `#Card Generation API` `#Marketing Image` `#Headless Browser` 
> A Node.js + Express + Puppeteer-based API for generating streamer light cards via headless browser automation. Accepts parameters like title, content, and QR codes to produce marketing images. Supports Docker deployment with retry mechanisms and concurrency management, ideal for batch marketing material generation.
> <sub>流光卡片API</sub>
...
> `#Browser-based OS` `#Static Architecture` `#Web Micro-apps` `#Device Collaboration` 
> NoneOS is a lightweight browser-based virtual operating system with pure static file architecture requiring no backend server. It offers file management, bookmark sync, note sync (Notion-like), and file transfer (LocalSend-like) applications, enabling seamless device collaboration. Developers can build web micro-apps using the ofa.js framework.
...
> `#Website Monitoring` `#SSL Certificate` `#Next.js` `#Docker Deployment` 
> CoolMonitor is an open-source monitoring tool with a modern UI, supporting HTTP/HTTPS website, API, HTTPS certificate expiration, TCP port, MySQL/Redis database monitoring and more. Built with Next.js and SQLite, it supports Docker one-click deployment with dark/light theme switching and multi-channel notifications.
...
> `#Geoid Model` `#EGM96` `#WGS84 Altitude Conversion` `#Java Library` 
> Java library computing EGM96 geoid offsets to convert WGS84 ellipsoidal altitudes to mean sea level (MSL) altitudes. Addresses the common GPS issue where receivers return ellipsoidal heights instead of orthometric heights useful for end users. Reads pre-computed EGM96 grid data and uses bilinear interpolation for offset calculation at any latitude/longitude. Primarily designed for Android Location API integration.
...
> `#Drone Cloud API` `#Docker Containerization` `#MQTT Real-time` `#DJI Integration` 
> A comprehensive DJI drone Cloud API demo project with Docker-optimized deployment, maintained by Spantree. Built with Vue.js frontend and Spring Boot backend, it leverages EMQX MQTT broker for real-time drone-to-cloud communication. Supports DJI Pilot 2 integration with a web interface for remote drone control and live data monitoring. Pre-configured MySQL and Redis containers streamline the integration testing workflow.
...
> `#Drone SDK` `#Android Development` `#UAV Control` `#DJI MSDK V5` 
> Official DJI Mobile SDK V5 sample project for Android, providing comprehensive APIs to control drone software and hardware interfaces. Supports multiple aircraft models including Mavic 3, Matrice 300/350 RTK, Mini series. Includes scenographic examples (uxsdk) and complete sample applications to help developers integrate aerial photography control, live video streaming, and waypoint missions efficiently.
...
> `#Drone Cloud API` `#DJI Pilot Integration` `#Docker Deployment` `#Deprecated` 
> DJI's official Cloud API sample project for demonstrating drone cloud API integration. This reference implementation helps developers understand API calling logic with Docker deployment support. The project was officially deprecated on April 10, 2025 with security warnings. Latest version is Cloud API 1.10.0, MIT licensed.
...
> `#HAT-trie` `#Prefix Search` `#String Indexing` `#C++ Data Structure` `#c-plus-plus` `#cpp` `#data-structures` `#hat-trie` `#header-only` `#trie` 
> A C++ implementation of the HAT-trie data structure based on the 2007 paper, providing htrie_map and htrie_set containers. It achieves low memory usage and efficient lookups by compressing common prefixes into array hash nodes. Supports prefix search, prefix erasure, longest prefix matching, and serialization. Ideal for large-scale string storage and autocomplete scenarios. Header-only library.
...
> `#Retrieval-Augmented Generation` `#Agent` `#Large Language Models` `#Context Engine` `#agentic-ai` `#agentic-retrieval` `#agentic-search` `#ai` `#ai-agents` `#context-engine` `#context-management` `#llm-apps` `#rag` `#retrieval-augmented-generation` 
...
> [!info]
> ⭐ 1,607 · Java · 2026-05-29  
> [GitHub](https://github.com/pagehelper-org/pagehelper-spring-boot)  
> `#Spring Boot Starter` `#MyBatis Pagination` `#PageHelper` `#Database Paging` 
> PageHelper-Spring-Boot-Starter integrates the PageHelper pagination plugin into Spring Boot, enabling MyBatis paging capabilities via Maven dependency. Supports PageHelper 6.x with advanced features including async count, automatic dialect detection, and extensible SQL parsers, compatible with Spring Boot 3.x and JDK17+.
...
> `#PDF Parsing` `#Document Structuring` `#LLM Preprocessing` `#ai4science` `#document-analysis` `#docx` `#extract-data` `#layout-analysis` `#ocr` `#parser` `#pdf` `#pdf-converter` `#pdf-extractor-llm` `#pdf-extractor-pretrain` `#pdf-extractor-rag` `#pdf-parser` `#pptx` `#python` `#xlsx` 
...
> `#GPU Graph Analytics` `#CUDA/ROCm` `#Fine-grained Load Balancing` `#High-Performance Computing` `#algorithm` `#algorithms` `#cpp` `#cuda` `#cxx` `#essentials` `#gnn` `#gpu` `#graph` `#graph-algorithms` `#graph-analytics` `#graph-engine` `#graph-neural-networks` `#graph-primitives` `#graph-processing` `#gunrock` `#hpc` `#parallel-computing` `#sparse-matrix` 
> Gunrock is a high-performance GPU graph analytics library supporting both NVIDIA CUDA and AMD ROCm/HIP platforms. It employs a frontier-centric, data-centric abstraction with fine-grained load balancing, achieving optimal balance between performance and programmability. Developers can rapidly develop new graph primitives with support for scaling from single to multi-GPU, ideal for large-scale graph computing like social network analysis and route planning.
...
> `#Geospatial Routing` `#Shortest Path Algorithm` `#PostGIS Extension` `#Network Analysis` `#network-analysis` `#osgeo` `#postgis` `#postgresql-extension` `#routing` `#spatial-analysis` 
> pgRouting extends PostGIS/PostgreSQL geospatial databases with routing capabilities, providing network analysis functions including shortest path, A* search, bidirectional Dijkstra, Yen's K-shortest paths, driving distance, and traveling salesman problem. Built on Boost Graph Library with customizable cost functions for navigation, logistics, and GIS applications.
...
> `#CUDA Parallel Computing` `#GPU-Accelerated Algorithm` `#A* Pathfinding` `#Massively Parallel Search` 
> A CUDA-based GPU-accelerated implementation of the A* pathfinding algorithm, based on the Massively Parallel A* Search paper. Provides a generic A* framework with example applications for sliding puzzle and 2D grid pathfinding. Leverages massive parallel computing capabilities of GPUs for optimized search performance.
...
> `#C# Desktop App` `#Android Remote Control` `#scrcpy GUI` `#Wireless Screen Mirror` `#adb` `#android` `#scrcpy` 
> FreeControl is a Windows desktop application built on scrcpy using C#, providing a simple GUI for wireless/USB remote control of Android devices from PC. It supports custom Scrcpy path, auto-lock on close, and UHID keyboard for optimized Chinese input, ideal for developers and users needing to control Android devices from computers.
...
> `#AST Analysis` `#Code Deobfuscation` `#Control Flow Flattening` `#JavaScript Reverse Engineering` `#javascript-deobfuscator` `#obfuscator` 
> A Babel-based AST analyzer for obfuscated JavaScript code, supporting stringArray restoration, dead code elimination, control flow flattening reversal, local code transformation, and self-defending code handling. Capable of processing obfuscated code from jjencode, sojson, obfuscator and similar tools, ideal for security research and code analysis.
...
> [!info]
> ⭐ 1,741 · Java · 2023-04-26  
> [GitHub](https://github.com/java-deobfuscator/deobfuscator) · [Website](https://javadeobfuscator.com)  
> `#Java Deobfuscation` `#Bytecode Analysis` `#Reverse Engineering` `#Obfuscator` 
> Java Deobfuscator is a powerful tool for deobfuscating Java bytecode, supporting major commercial obfuscators like Zelix Klassmaster, Allatori, DashO, and DexGuard. It automatically detects obfuscation types and applies corresponding Transformers to restore obfuscated code control flow and strings, providing essential support for the open-source reverse engineering community.
...
> `#JavaAgent` `#Groovy Script` `#JVM Monitoring` `#Dynamic Debugging` 
> A runtime console tool for Java developers that injects into target JVM processes via JavaAgent without code modification. It provides a built-in Web management interface for executing Groovy scripts to perform dynamic code debugging, runtime variable inspection, and business logic verification.
...
> `#Distributed Systems` `#Microservices` `#Service Mesh` `#Software Architecture` `#architecture` `#cloud-native` `#istio` `#kubernetes` `#microservice` `#springcloud` 
> Phoenix Architecture is an open-source technical documentation focusing on building reliable distributed software systems, covering modern architectural patterns including microservices, service mesh, and serverless. It provides companion demo projects and systematically organizes design principles and practices for enterprise-level distributed systems.
...
> `#Administrative Division` `#Geo-coordinates` `#GIS Data` `#Multi-level Linkage` 
> A comprehensive China administrative division data collection project providing province/city/district/township-level data from multiple sources including NBS, Ministry of Civil Affairs, Amap and Tencent Maps. Features CSV files with pinyin annotations, coordinates and boundary ranges, supporting conversion to JSON/multi-level JS, with tools exporting to shp, geojson, sql formats for map visualization and geolocation applications.
...
> `#Stream Processing` `#Real-time Monitoring` `#High Concurrency` `#Multi-dimensional Analytics` `#analytics` `#big-data` `#clickhouse` `#digital-solutions` `#flink` `#spark` `#statistics` 
> XL-LightHouse is a real-time business monitoring system for ultra-large-scale data processing, capable of handling 10 million metrics and 500 billion daily messages. It provides stream processing operations including count, sum, avg, distinct, and topN/lastN, with multi-dimensional analysis across minute, hour, and day time granularities. Features one-click deployment, standalone/cluster modes, and API integration.
...
> `#Docker Containerization` `#qBittorrent NOX` `#Headless Download Client` `#Alpine Linux` `#alpine` `#container` `#debian` `#docker` `#nox` `#qbittorrent` 
> Dockerized qBittorrent NOX (headless) with remote Web UI for BitTorrent downloads. Supports multiple version tags (3.x/4.x), Alpine-based latest image for minimal size, runs as non-root user with dumb-init for security hardening. Ideal for private download servers or home media centers.
...
> `#BuildKit` `#Dockerfile Debugging` `#Interactive Debugger` `#DAP Protocol` 
> buildg is an interactive Dockerfile debugger built on BuildKit, enabling source-level inspection, breakpoints, and step-by-step execution. Developers can pause during build and access interactive shell with custom debugging tools. Supports VS Code, Emacs, and Neovim via DAP protocol for visual debugging.
...
> `#Kubernetes Dashboard` `#Multi-cluster Management` `#RBAC Access Control` `#Container Orchestration` `#1panel` `#dashboard` `#docker` `#k8s` `#kubernetes` `#panel` 
> KubePi is a modern Kubernetes dashboard developed by the 1Panel team, supporting multi-cluster import and management with RBAC-based permission control. It allows administrators to assign specific cluster/namespace access to users, helping developers efficiently manage applications and troubleshoot issues across Kubernetes clusters. Features one-click Docker deployment and intuitive Web UI.
...
> `#Vector Tile Parsing` `#Mapbox MVT` `#Protobuf Decoding` `#Web GIS` 
> mapbox/vector-tile-js is an official JavaScript library for parsing Mapbox Vector Tiles (MVT). It decodes protobuf-encoded vector tile data, providing access to layers, features, and geometry with methods like loadGeometry(), bbox(), and toGeoJSON(). Essential for web mapping and GIS applications.
...
> `#Proxy Kernel` `#mihomo` `#sing-box` `#Automation` `#adguard-home` `#adguardhome` `#clash` `#clash-meta` `#clash-premium` `#clashmeta` `#dashboard` `#mihomo` `#proxy` `#shellcrash` `#sing-box` `#singbox` 
> An automated tool for synchronizing and building proxy tool kernels, updating daily at midnight: mihomo (Meta/Alpha), sing-box (Release/Dev/reF1nd), Dashboard panels, and AdGuard Home. Uses UPX compression and supports multiple architectures, providing convenient kernel upgrade solutions for proxy tools like ShellCrash.
...
> `#Docker Container` `#Clash Proxy` `#Web Dashboard` 
> A Docker image project bundling Clash proxy core with a web management dashboard in a single container. Modified backend connects directly to port 9090 within the container, eliminating cross-container configuration. Ideal for users needing quick proxy service deployment.
...
> `#Doc Sharing` `#Real-time Analytics` `#Next.js` `#dataroom` `#next-auth` `#nextjs` `#open-source` `#pdf` `#postgresql` `#prisma` `#tailwindcss` `#typescript` `#zod` 
> Papermark is an open-source DocSend alternative for secure document sharing with custom domains and real-time analytics. Built on Next.js & TypeScript, it can be self-hosted for pitch decks and reports.
...
> [!info]
> ⭐ 3,794 · Java · 2025-12-23  
> [GitHub](https://github.com/blossom-editor/blossom) · [Website](https://www.wangyunf.com/blossom-doc/index)  
> `#Bi-directional Notes` `#Self-hosted` `#Markdown Editor` `#Knowledge Management` `#blog` `#client-server` `#editor` `#markdown` 
> Blossom is a self-hostable cloud-based bi-directional note-taking application with Markdown support, enabling multi-device sync across Windows, Mac and web clients. Features built-in image management, calendar planning, todos, Pomodoro timer, and dynamic blogging—ideal for personal knowledge management and team collaboration.
...
> `#Shell Script` `#DDNS` `#Embedded Systems` `#IPv6` `#cloudflare` `#cloudflare-ddns` `#ddns` `#dnspod` `#dnspod-ddns` `#godaddy` `#godaddy-ddns` `#shell-script` 
> UE-DDNS is a universal embedded DDNS shell script with modular minimalist dependency design. Supports IPv4/IPv6 dual-stack and obtains dynamic IPs from URLs or network interfaces. Compatible with most Unix platforms including busybox-only embedded devices. Features an interactive wizard where users only need to provide API tokens to generate custom scripts. Supports major DNS providers like Cloudflare, Dnspod, and Godaddy with strict IP comparison to avoid duplicate requests.
...
> [!info]
> ⭐ 35,896 · Go · 2026-06-16  
> [GitHub](https://github.com/1Panel-dev/1Panel) · [Website](https://1panel.pro)  
> `#VPS Control Panel` `#Container Management` `#OpenClaw Deployment` `#SSL Automation` `#agent` `#clawdbot` `#copaw` `#docker` `#docker-ui` `#hermes` `#hermes-agent` `#linux` `#lnmp` `#ollama` `#openclaw` `#openresty` `#qwenpaw` `#webmin` 
...
> `#Modbus Protocol` `#Java Implementation` `#Industrial Automation` `#IoT Communication` 
> modbus4j is a high-performance Modbus protocol library implemented in Java by Infinite Automation Systems. It supports ASCII, RTU, TCP, and UDP transport modes, operating as master or slave, with automatic request partitioning and response data type parsing. Ideal for industrial automation, SCADA systems, and IoT device monitoring.
...
> `#Edge Computing` `#Data Collection Agent` `#Apache NiFi Ecosystem` `#Lightweight Dataflow` `#hacktoberfest` `#java` `#minifi` `#nifi` 
> Apache MiNiFi is a subproject of Apache NiFi designed as a lightweight edge agent for data collection at the source. It operates immediately adjacent to sensors, systems, or servers with a small footprint, central management capabilities, and full data provenance tracking, seamlessly integrating with NiFi for end-to-end dataflow management.
...
> `#Regex Generation` `#Rust Library` `#CLI Tool` `#Python Bindings` `#cli` `#command-line-tool` `#python` `#python-library` `#regex` `#regex-pattern` `#regexp` `#regular-expression` `#regular-expressions` `#rust` `#rust-cli` `#rust-crate` `#rust-library` `#terminal` `#tool` 
> grex is a Rust-based tool that automatically generates regular expressions from user-provided test cases. It provides both a CLI and Python bindings. As a Rust port of regexgen, it supports cross-platform deployment and can generate either the most specific or more generalized regex patterns.
...
> `#Hydrology Monitoring` `#Netty` `#SL651-2014` `#Industrial IoT` `#java` `#netty4` `#sl651-2014` `#springboot` 
> A Netty-based implementation of SL651-2014 hydrology communication protocol server, supporting M1/M2/M4 communication modes with built-in 2F link maintenance and 32 periodic reporting function codes. Features TCP/UDP modes, CRC verification, and packet segmentation to handle粘包分包 issues. Extensible business logic for hydrology monitoring data transmission.
...
> `#Rust` `#Terminal Emulator` `#Productivity Theater` `#Cross-platform CLI` `#cli` `#fake` `#hacktoberfest` `#nonsense` `#useless` `#wasm` `#webassembly` 
> A Rust-based terminal activity simulator that displays fake high-load operations like kernel compilation, Docker builds, and crypto mining to help users appear busy. Supports Linux, macOS, Windows, and WebAssembly with multiple simulated scenarios for legitimate 'productivity theater'.
...
> [!info]
> ⭐ 10,672 · Vue · 2026-06-16  
> [GitHub](https://github.com/jason5ng32/MyIP) · [Website](https://ipcheck.ing)  
> `#Network Diagnostics` `#IP Geolocation` `#DNS Leak Test` `#WebRTC Detection` `#awesome` `#censorship` `#dns` `#dnsleak` `#ip` `#ipinfo` `#leaks` `#myip` `#network` `#pingchecker` `#proxy` `#security` `#security-tools` `#speedtest` `#webrtc` `#whatismyip` `#whatismyipaddress` `#whois` `#whois-lookup` 
...
> `#Kanban Board` `#Open Source PM` `#Self-hosted` `#Go Backend` `#asana` `#collaboration` `#goal-tracking` `#golang` `#hacktoberfest` `#kanban-board` `#notion` `#project` `#project-management` `#trello` 
> Focalboard is an open source project management tool by the Mattermost community, featuring Kanban-style task organization. As an open source alternative to Trello, Notion, and Asana, it supports multilingual interfaces and cross-platform deployment (Windows/Mac/Linux) with self-hosting capability. The desktop edition uses embedded SQLite, while the server edition enables multi-user collaboration for personal and team project management.
...
> `#In-Memory Database` `#Windows Port` `#NoSQL Database` `#Key-Value Store` `#redis` `#redis-for-windows` `#redis-msi-installer` `#redis-windows` `#redis-windows-installer` `#redis-windows-native` `#redis-windows-service` 
> An unofficial native port of Redis (versions 4.0.14 and 5.0.14) for Windows x64. Built upon the MS Open Tech fork, compiled with Visual Studio 2019, featuring customized jemalloc allocator to enable fork-based RDB/AOF persistence on Windows. Ideal for enterprise applications requiring Redis on Windows infrastructure.
...
> `#Mixture-of-Experts` `#Large Language Model` `#JAX Framework` `#xAI` 
> xAI's open-source 314B parameter Mixture-of-Experts LLM with 8 experts, activating 2 per token. Built with JAX, supporting RoPE embeddings, activation sharding, and 8-bit quantization. Context length up to 8,192 tokens. Weights available via HuggingFace or BitTorrent under Apache 2.0 license.
...
> `#JavaEE Architecture` `#Low-code Platform` `#Workflow Engine` `#Trusted IT Certification` 
> O2OA is a 100% open-source enterprise collaboration platform built on JavaEE distributed architecture. It offers low-code development capabilities including workflow engine, form customization, and page customization with cross-platform mobile office support. Certified for China's trusted IT ecosystem, it supports domestic operating systems like Kylin and databases such as Dameng and Kingbase, ideal for government OA and military information systems.
...
> `#PostgreSQL Extension` `#Chinese Segmentation` `#Full-text Search` `#SCWS` `#chinese` `#chinese-nlp` `#chinese-text-segmentation` `#extension` `#postgresql` `#scws` `#zhparser` 
> zhparser is a PostgreSQL extension for Chinese full-text search, implementing a Chinese language parser based on SCWS (Simple Chinese Word Segmentation). It supports custom dictionaries and multiple segmentation modes, enabling vectorization and querying of Chinese text. Compatible with PostgreSQL 9.2 and above.
...
> [!info]
> ⭐ 6,432 · TypeScript · 2026-06-12  
> [GitHub](https://github.com/xushengfeng/eSearch) · [Website](https://esearch-app.netlify.app/)  
> `#Offline OCR` `#Screen Overlay` `#Electron App` `#Scrolling Screenshot` `#clipboard` `#color-picker` `#cross-platform` `#electron` `#image-editing` `#image-editor` `#linux` `#live-text` `#macos` `#ocr` `#paddleocr` `#recorder` `#screen-capture` `#screen-recorder` `#screenshot` `#search` `#search-photos` `#translate` `#translator` `#windows` 
> eSearch is a cross-platform screenshot and OCR tool featuring offline text recognition, screen image overlay, reverse image search, scrolling screenshots, screen recording, and real-time screen translation. Built with Electron and PaddleOCR, it supports Windows, Linux, and macOS for efficient screenshot editing, information extraction, and multilingual translation.
...
> [!info]
> ⭐ 8,793 · Java · 2026-06-16  
> [GitHub](https://github.com/apache/shenyu) · [Website](https://shenyu.apache.org/)  
> `#API Gateway` `#Microservices Governance` `#Service Proxy` `#Plugin Hot-Swapping` `#api-gateway` `#dubbo-proxy` `#grpc-proxy` `#http-proxy` `#reactor-java` `#sofa-proxy` `#spring-cloud-proxy` 
...
> `#Markdown Viewer` `#Obsidian Publishing` `#PHP` `#Static Site Generator` `#markdown` `#obsidian` `#parsedown` `#perlite` 
> Perlite is a PHP-based web markdown viewer optimized for Obsidian vault publishing. No database required—it auto-builds sites from Markdown folder structures as an open-source alternative to Obsidian Publish. Features include theme support, interactive graph visualization, LaTeX, Mermaid diagrams, and full-text search.
...
> `#Diffusion Model` `#Visual Text Generation` `#Multilingual Rendering` `#Image Editing` 
> AnyText is a diffusion-based multilingual visual text generation and editing framework, featuring an auxiliary latent module and text embedding module to produce text seamlessly integrated with backgrounds. It uses text-control diffusion loss and text perceptual loss to enhance writing accuracy, supporting generation, editing and community model weight merging. Accepted as ICLR 2024 Spotlight.
...
> `#JDBC Benchmarking` `#Database Performance` `#TPC-C/TPC-H` `#Multi-threaded Load Testing` `#database` 
> BenchBase is a multi-DBMS SQL benchmarking framework developed by CMU's database group, supporting standardized performance evaluation across PostgreSQL, MySQL, CockroachDB via JDBC. It includes 15+ benchmarks like TPC-C, TPC-H, YCSB, with multi-threaded load generation and granular performance metrics for database comparison and optimization research.
...
> `#Axum Web Framework` `#Sea-Query ORM` `#Multi-tenant Architecture` `#Declarative Macros` 
> A production-ready Rust web application template built with Axum, featuring Sea-Query ORM and modql for flexible query filtering. Includes declarative CRUD macros to reduce boilerplate, ModelManager with database transaction support, and demonstrates multi-tenant architecture using Org/Space constructs for building enterprise collaboration platforms.
...
> `#SQL-driven` `#Low-code Platform` `#Web UI Generation` `#Database Tools` `#database` `#framework` `#mysql` `#postgresql` `#server` `#serverless` `#sql` `#sqlite` `#web` 
> SQLPage is an SQL-only web application builder that automatically generates interactive data interfaces from simple .sql query files. It supports SQLite, PostgreSQL, MySQL, SQL Server and various ODBC databases, providing UI components like lists, charts, forms, and cards for rapid data application development without frontend coding.
...
> [!info]
> ⭐ 9,778 · Java · 2026-06-15  
> [GitHub](https://github.com/apache/cassandra) · [Website](https://cassandra.apache.org/)  
> `#Distributed Database` `#NoSQL Database` `#Wide-Column Store` `#Partitioned Storage` `#cassandra` `#database` `#java` 
> Apache Cassandra is a highly scalable distributed NoSQL database featuring partitioned row storage that transparently distributes data across multiple machines. With a SQL-like CQL query language, it offers high availability, no single point of failure, and multi-datacenter deployment support, ideal for large-scale distributed applications requiring resilient data storage.
...
> `#Log Desensitization` `#Data Security` `#Java Annotation` `#Privacy Protection` `#dfa` `#fastjson` `#java` `#java-annotation` `#json` `#log` `#log4j2` `#log4j2-plugin` `#logback` `#security` `#sensitive` `#sensitive-data-security` `#slf4j` 
> Annotation-based Java log desensitization framework that elegantly masks sensitive information including phone numbers, emails, bank cards, IDs, and passwords. Features 10+ built-in strategies, custom hashing, and condition-based desensitization. Uses log4j2/logback plugins for unified desensitization, outperforming regex matching. Designed for financial security scenarios.
...
> `#Hikvision SDK` `#RTSP Streaming` `#Spring Boot Starter` `#Video Surveillance` 
> A Spring Boot Starter封装 for Hikvision SDK, providing device login, real-time preview, playback and screenshot capabilities. It solves web browser compatibility and high latency issues via RTSP streaming, supporting direct FLV output without nginx. Ideal for multi-platform video surveillance integration.
> <sub>海康威视sdk接入</sub>
...
> `#Java DSL` `#REST API Testing` `#Response Validation` `#Spring Integration` `#groovy` `#http` `#java` `#json` `#kotlin` `#rest` `#rest-api` `#test` `#test-automation` `#xml` 
> REST Assured is a Java DSL that simplifies REST API testing by bringing the expressiveness of dynamic languages into the Java domain. It supports JSON/XML response validation, parameters, headers, cookies and body assertions, includes built-in JsonPath and XmlPath parsers, offers multiple authentication mechanisms like Basic and OAuth, and integrates with Spring Framework for MockMvc testing.
...
> `#Declarative HTTP Client` `#Java HTTP Framework` `#Annotation-Driven` `#Spring Boot Integration` `#declarative-http-client` `#fegin` `#http` `#http-client` `#https` `#java-http-client` `#request` `#rest` `#restful` `#retrofit` `#socks` 
> Forest is a declarative HTTP client framework for Java that transforms HTTP requests into local interface method calls via annotations, decoupling business logic from HTTP protocols. Supporting dual backend engines (HttpClient/OkHttp), it provides enterprise features including JSON/XML/Protobuf serialization, interceptors, and OAuth2 authentication with seamless Spring Boot integration.
...
> `#Stable Diffusion` `#Prompt Engineering` `#AI Art` `#Multi-language Translation` 
> A stable-diffusion-webui extension enhancing prompt input with visual tokenization, syntax highlighting, and batch operations. Features auto-translation via multiple APIs, history tracking, and bookmarking to streamline AI art generation workflows.
...
> `#Stable Diffusion Extension` `#Image Auto-Tagging` `#DeepDanbooru` `#WD14 Tagger` 
> An automatic image tagging extension for Automatic1111's Stable Diffusion WebUI, supporting multiple models including Waifu Diffusion 1.4 Tagger and DeepDanbooru. Interrogates single or multiple images with booru-style tags, extracting visual features for AI art workflow preprocessing and content annotation.
...
> `#GraalVM native-image` `#Maven Build` `#Java Native Compilation` `#Containerized Build` `#docker-image` `#graalvm` `#graalvm-native-image` `#maven` 
> Debian-based Docker image with GraalVM CE (JDK 11/17/21/25) and Maven for compiling Java projects into native Linux executables via native-image. Auto-updated weekly with security patches, includes Docker client, Git and other dev tools, supports non-root execution (uid 1000).
...
> [!info]
> ⭐ 1,976 · Java · 2026-06-15  
> [GitHub](https://github.com/rememberber/MooTool) · [Website](https://mootool.luoboduner.com/)  
> `#JavaFX Desktop App` `#Developer Toolkit` `#Cross-platform Tool` `#HTTP Client` `#desktop-app` `#gui` `#httpclient` `#httprequest` `#java` `#jsonformat` `#qrcode` `#quicknote` `#swing` `#swing-gui` `#switchhost` `#tool` 
> MooTool is a cross-platform developer toolkit featuring code note-taking with syntax highlighting, JSON processing, HTTP requests, encoding/decoding, encryption/decryption, timestamp conversion, QR codes, calculator, network tools, and color picker. Built with JavaFX, it runs on Windows, Linux, and macOS.
...
> `#Multithreading` `#In-Memory Database` `#Redis Fork` `#High Throughput` 
> KeyDB is a high-performance Redis fork developed by Snap Inc, focusing on multithreading, memory efficiency, and high throughput. Beyond performance gains, it offers Active Replication, FLASH storage, and subkey expires. Its MVCC architecture enables non-blocking KEYS and SCAN operations while maintaining full Redis protocol and script compatibility as a seamless drop-in replacement.
...
> `#Dynamic Datasource` `#Read-Write Separation` `#Spring Boot Starter` `#Distributed Transaction` `#datasource` `#distributed-transaction` `#dynamic-datasource` `#dynamicdatasource` `#multi-datasource` `#multidatasource` `#mybatis` `#spring-boot` `#springboot` 
> dynamic-datasource-spring-boot-starter is a Spring Boot component by baomyelitis for dynamic multi-datasource management. Key features include datasource grouping (read-write separation, master-slave, hybrid mode), sensitive config encryption, dynamic add/remove datasources, nested switching, and Seata-based distributed transaction support. Compatible with Spring Boot 1.5.x to 4.x, integrated with Druid, HikariCp, BeeCp.
...
> `#MQTT Protocol` `#IoT Messaging` `#Light Message Queue` `#Mobile Push` 
> RocketMQ-MQTT extends Apache RocketMQ with MQTT protocol support, enabling message ingestion from IoT devices and mobile apps. Built on RocketMQ's unified storage engine with Light Message Queue (LMQ) integration, ideal for IoT and mobile messaging scenarios.
...
> `#Java Caching` `#Distributed Cache` `#Two-Level Cache` `#Spring Boot Integration` `#cache` `#java` `#jcache` `#redis` `#spring` `#spring-cache` 
> JetCache is an Alibaba open-source Java cache abstraction framework providing unified Cache API and declarative caching annotations more powerful than Spring Cache. It supports TTL, two-level caching, distributed auto-refresh and distributed locking, with implementations for Redis, Caffeine, and more. Features include cache statistics, custom serialization, and Spring Boot integration.
...
> `#Time Series Database` `#Industrial IoT` `#Performance Benchmark` `#TSDB` 
> IoT-Benchmark is an open-source benchmarking tool designed for evaluating time series database performance in Industrial IoT scenarios. It supports 12+ mainstream TSDBs including IoTDB, InfluxDB, TimescaleDB, and TDengine, providing comprehensive testing capabilities such as data generation, write/query testing, correctness verification, and result visualization.
...
> `#Distributed Task Scheduling` `#Workflow Engine` `#Delay Task` `#Sharding Algorithm` `#delay` `#job` `#schedule` `#task` 
> Openjob is a high-performance distributed task scheduling framework using Master/Worker architecture with consistent sharding algorithm for lock-free design. It supports cron jobs, second-level tasks, delay tasks, workflow scheduling, and lightweight distributed computing. Features visual DAG workflow, permission management, alarm monitoring, and task log storage (H2/MySQL/Elasticsearch). Supports Java, Go, PHP, Python with MySQL/PostgreSQL/Oracle compatibility and unlimited horizontal scaling.
...
> `#Workflow Scheduling` `#Job Orchestration` `#Java` `#ETL Pipeline` `#azkaban` `#hacktoberfest` `#scheduling` `#workflow-engine` 
> Azkaban is an open-source workflow scheduling system developed by LinkedIn. It manages complex job dependencies, provides scheduled execution and visual monitoring through a web-based UI. Built with Java, it's ideal for data pipeline orchestration, ETL job scheduling, and batch processing workflows.
...
> `#Rapid API Development` `#Script Engine` `#Web-based Coding` `#Dynamic Compilation` `#api` `#api-framework` `#automatic-api` `#http-api` `#http-api-generator` `#magic-api` `#magic-script` `#restful-api` `#sql` 
> magic-api is a Java-based rapid API development framework that automatically generates HTTP interfaces through web-based script editing, eliminating the need for traditional Java classes. Supporting multiple databases like MySQL, PostgreSQL, and Redis, it uses magic-script engine for dynamic compilation and real-time publishing, offering enterprise features like code completion, online debugging, and transaction support to dramatically improve API development efficiency.
...
> `#Browser Extension` `#GitHub Tools` `#Tech Stack Detection` `#Developer Utility` `#chrome-extension` `#development-experience` `#extension` `#firefox-addon` `#github` `#techstack` 
> Tech Stack is a browser extension that automatically identifies and displays the tech stack of GitHub repositories when users visit them. By analyzing code structure and dependency configuration files, it helps developers quickly understand project technologies. Supports Chrome, Firefox, and Edge, free and open source forever.
...
> `#PDF Processing` `#Self-Hosted` `#Enterprise PDF` `#Document Automation` `#docker` `#hacktoberfest` `#java` `#pdf` `#pdf-converter` `#pdf-editor` `#pdf-manipulation` `#pdf-merger` `#pdf-ocr` `#pdf-tools` `#pdf-web-apps` `#pdfmerger` `#self-hosted` 
...
> `#Nginx Module` `#Health Check` `#Load Balancing` `#Failover` 
> An Nginx module providing active health checking for upstream servers. Supports multiple protocols including TCP, HTTP, SSL_HELLO, MySQL, AJP, and FastCGI with configurable check interval, fall/rise counts and timeout. Automatically marks failed servers as down, enabling automatic failover and improving system availability.
...
> `#Distributed Transaction` `#TCC Pattern` `#Financial-grade` `#Microservices Consistency` `#dubbo` `#motan` `#spring` `#springcloud` `#tac` `#tansaction` `#tcc` `#xa` 
> Hmily is a financial-grade distributed transaction solution implementing TCC (Try-Confirm-Cancel) pattern. It offers zero-intrusive Spring-Boot/Spring-Namespace integration with support for mainstream RPC frameworks including Dubbo, SpringCloud, and Motan. Features high reliability (automatic rollback, timed recovery), high performance (decentralized cluster-native design), and observability (metrics monitoring + admin UI). Storage backends include MySQL, Oracle, MongoDB, Redis, and Zookeeper, ideal for cross-service transaction consistency in microservices architectures.
...
> `#Daemonless Containers` `#Rootless Security` `#OCI Runtime` `#containers` `#docker` `#kubernetes` `#linux` `#oci` 
> Podman is a daemonless, rootless-capable OCI container and pod manager with Docker-compatible CLI and REST API. Built on libpod library, it supports full container lifecycle management across Linux/macOS/Windows, featuring CRIU checkpoint/restore, Netavark networking, and enhanced security without a background daemon.
...
> `#ClickHouse` `#Log Analytics` `#Go` `#Observability` `#admin` `#analytics` `#clickhouse` `#clickhouse-database` `#elk` `#go` `#golang` `#kibana` `#kubernetes` `#log` `#search` `#ui` 
> ClickVisual is a lightweight log analytics and visualization platform built on ClickHouse, supporting SQL log queries, visual dashboards, alarm processing, and DAG workflows with VSCode-style configuration panel, GitHub/GitLab auth, and kubectl one-click deployment.
...
> `#DevOps Automation` `#CI/CD Pipeline` `#Docker Management` `#Project Monitoring` `#ci-cd` `#docker` `#java` `#jpom` `#ssh-client` 
> A lightweight, low-intrusive online build, automated deployment, operations management and project monitoring tool. It provides CI/CD pipelines, Docker container management, SSH terminal, nginx config management, multi-user permission control, and automatic anomaly alerting for dev teams.
...
> `#SMS Aggregation Framework` `#Spring Boot Starter` `#Multi-vendor Integration` `#Unified API` `#java` `#sms` `#solon` `#springboot` 
> SMS4J is an SMS aggregation framework developed by Dromara, built on Spring Boot Starter. It unifiedly integrates 20+ SMS providers including Alibaba Cloud, Tencent Cloud, and Huawei Cloud through a consistent API. Developers can switch vendors by configuring YAML without code changes, significantly reducing integration costs and maintenance complexity.
...
> `#React Frontend` `#Chrome Extension` `#GitHub Star Ranking` `#Time Travel` `#chrome-extension` `#github-extension` `#trending-repositories` `#trending-topics` 
> A React-based explorer for GitHub’s most-starred projects, letting you travel back in time, filter by language, and toggle list/grid views—available as both a web app and Chrome extension.
...
> `#Spring Cache` `#Redis Cache` `#Delayed Eviction` `#High Concurrency` `#spring` `#spring-boot-starter` `#spring-cache` 
> A Spring cache enhancement framework that solves cache stampede and consistency issues in high concurrency scenarios via delayed cache eviction. Non-invasive to business code, simply configure cacheManager in @CacheEvict to enable delayed cache clearing with debug logging support.
...
> `#Java ORM` `#Database Sharding` `#Cache Translation` `#OLAP Analytics` `#orm` `#sagacity` `#sagacity-orm` `#sagacity-sqltoy` `#sql` `#sql-orm` `#sqltoy` `#sqltoy-orm` `#sqltoy-sql` 
> Sagacity-SQLToy is an intelligent Java ORM framework that deeply integrates JPA functionality with powerful query capabilities. It features innovative cache translation to replace LIKE queries, cross-database SQL auto-adaptation, complex analytics including group summary, pivot/unpivot, YoY/MoM analysis, and tree aggregation. With sharding, multi-tenancy, data masking, and high-concurrency updateFetch, it excellently supports OLAP scenarios like ClickHouse and Doris.
...
> `#AutoJS` `#Check-in Automation` `#Reward Collection` `#Smart Scheduling` 
> An Auto.js-based automation platform aggregating daily check-in tasks from 20+ platforms including JD, Alipay, Taobao, miHoYo, and Dingdong. Features visual configuration, intelligent randomized scheduling, and flexible grouping for fully automated reward collection with extensible custom script support.
...
> `#Database Management` `#SQL Client` `#Eclipse RCP` `#JDBC Driver` `#ai` `#database` `#databricks` `#db2` `#dbeaver` `#erd` `#gui` `#java` `#jdbc` `#mysql` `#nosql` `#oracle` `#postgresql` `#redshift` `#snowflake` `#sql` `#sqlite` `#sqlserver` 
...
> `#Distributed Object Storage` `#OpenStack` `#Multi-tenant Storage` `#High Concurrency` `#service` 
> OpenStack Swift is a distributed object storage system scaling from single machine to thousands of servers. Optimized for multi-tenancy and high concurrency with a full RESTful API. Ideal for backups, web/mobile content, and unstructured data storage with high availability and automatic fault tolerance.
...
> `#Rust Full-stack Framework` `#Spec-driven Development` `#Composable Architecture` `#ORM Database` `#asynchronous` `#http` `#orm` `#rust` `#web` 
> Zino is a spec-driven full-stack Rust framework with modular composable architecture, featuring built-in ORM, scheduler, unified LLM access, and OpenAPI generation, supporting actix-web, axum, and dioxus for rapid application development.
...
> `#WinUI3` `#GitHub Client` `#Fluent Design` `#.NET 8` `#client` `#csharp` `#developer-tools` `#fluent` `#fluenthub` `#github` `#github-api` `#graphql` `#hacktoberfest` `#mica` `#mit-license` `#modern` `#oauth` `#power-user` `#powerful` `#uwp` `#windows` `#windows-11` `#winui` `#xaml` 
> FluentHub is a stylish yet powerful GitHub client for Windows built on WinAppSdk/WinUI3 with FluentUI design language. It features multi-tab multitasking, powerful page navigation, and seamless GitHub API integration for creating issues and pull requests directly within the app.
...
> `#Docker Containerization` `#Stable Diffusion` `#AI Image Generation` `#docker` `#docker-compse` `#gradio` `#pytorch` `#stable-diffusion` 
> This project provides Docker containerization for running Stable Diffusion AI image generation locally with minimal setup. It supports both AUTOMATIC1111 and ComfyUI web interfaces, enabling text-to-image and image-to-image generation through an intuitive browser-based UI.
...
> `#Distributed Two-Level Cache` `#Caffeine Cache` `#Hot Key Detection` `#Hybrid Cache Architecture` 
> L2Cache is a distributed two-level caching framework combining Caffeine (L1) and Redis (L2) with hybrid mode support. It provides automatic hot-key detection, dynamic cache configuration, cache penetration/breakdown solutions, and batch operations. Production-validated with 35W QPS capability, ideal for core services like e-commerce, user management, and marketing platforms.
...
> `#Knowledge Management` `#Bidirectional Linking` `#Self-hosted` `#Privacy-first` `#ai-agent` `#digital-garden` `#electron` `#knowledge-base` `#knowledge-graph` `#local-first` `#markdown` `#note-taking` `#notebook` `#notes-app` `#pdf` `#personal-knowledge-base` `#personal-knowledge-management` `#pkm` `#s3` `#self-hosted` `#siyuan` `#webdav` `#wiki` 
...
> `#Annotation-Driven` `#Data Population` `#Field Mapping` `#Spring Boot Integration` `#annotation` `#dict` `#fillp` `#java` `#spring` `#spring-boot` 
> crane4j is an annotation-driven data filling framework that automatically populates associated fields based on foreign keys or encoded values through simple annotation configuration. It supports multiple data sources including enumerations, key-value caches, and method calls, providing advanced features like template mapping, nested object filling, and sorting/grouping. Deeply integrated with Spring Boot, it enables effortless one-to-one to many-to-many complex field mapping, effectively eliminating repetitive boilerplate code in data assembly.
...
> `#GPT-3.5/4` `#Web Chat UI` `#G4F Integration` `#API-Free Access` `#apifree` `#chatgpt` `#chatgpt-api` `#chatgpt-clone` `#freegpt` `#freegpt4` `#gpt-4` `#gpt-4-api` `#gpt-4-free` `#gpt-free` `#gpt-interface` `#gpt3` `#gpt3-api` `#gpt3-turbo` `#gpt4` `#gpt4all` `#gptfree` `#jailbreak` `#language-model` `#website` 
> A ChatGPT-style Web UI powered by gpt4free (G4F) API, enabling free access to GPT-3.5/4 without requiring an API key. Features a user-friendly interface, Docker deployment support, and demo hosting on Replit and Hugging Face. The archived project remains as a reference for G4F web integration.
...
> `#Industrial Communication Protocol` `#Netty Framework` `#Modbus TCP/RTU` `#Java 17` `#java` `#modbus` `#modbus-rtu` `#modbus-security` `#modbus-serial` `#modbus-tcp` `#modbus-tls` 
> A modern Java 17+ implementation of Modbus protocol supporting Modbus TCP, Modbus TCP Security (TLS), Modbus RTU Serial, and Modbus RTU over TCP. Built on Netty for high-performance async communication with full client/server capabilities, covering all 11 Modbus function codes with support for custom PDUs and pluggable codecs.
...
> `#JavaFx` `#Desktop Toolkit` `#Development Debug` `#Plugin System` `#java` `#javafx` `#jfoenix` `#tools` 
> A cross-platform desktop toolkit built with JavaFx, featuring 40+ practical tools for developers including Http/Socket debugging, JSON formatting, encoding conversion, encryption/decryption, Cron generator, Redis/Kafka/Zookeeper clients, image processing, and Excel operations. Supports plugin extension, built with JDK17+/Gradle, UI uses Controlsfx and JFoenix libraries.
...
> `#SonarQube` `#i18n/l10n` `#Code Quality Management` `#Chinese Localization` 
> The official Chinese localization pack for SonarQube code quality management platform, providing complete interface translation. Maintained long-term with broad version compatibility from SonarQube 3.x to 26.x, enabling seamless Chinese localization for menus, reports, and rule descriptions to enhance user experience.
...
> `#TDengine` `#Time-series Database` `#Desktop Manager` `#Cross-platform` 
> TDengineGUI is a lightweight cross-platform desktop manager for TDengine time-series database, supporting Linux, Windows, and macOS. It provides database connection management, table/supertable browsing, SQL execution, and data filtering, enabling developers to observe and manipulate TDengine data more intuitively.
...
> `#IntelliJ IDEA Plugin` `#Mybatis SQL Formatting` `#Log Parsing` `#MybatisPlus Support` `#idea` `#idea-plugin` `#intellij-plugin` `#mybatis` `#mybatis-log` `#mybatis-log-plugin` 
> This IntelliJ IDEA plugin restores Mybatis SQL logs to executable SQL statements. Supports Mybatis 3.2.0+, MybatisPlus, auto-parses parameters and formats output, compatible with all major databases. Requires IDEA 2020.1+ and JDK 8+.
...
> `#Java Testing` `#Docker Containers` `#Integration Testing` `#JUnit` `#docker` `#docker-compose` `#hacktoberfest` `#integration-testing` `#java` `#junit` `#jvm` `#test-automation` `#testing` 
> Testcontainers is a Java library providing lightweight, throwaway Docker container instances for JUnit tests. It supports common databases like MySQL, PostgreSQL, MongoDB, and Selenium browsers, enabling rapid setup of isolated integration test environments. It is a widely-adopted testing tool in the Java ecosystem for containerized test infrastructure.
...
> `#Time-series Database` `#Distributed Database` `#Cloud-Native` `#Rust` `#cloud-native` `#distributed-database` `#horaedb` `#iot-database` `#prometheus-remote-storage` `#rust` `#timeseries-analysis` `#timeseries-database` `#tsdb` 
> Apache HoraeDB is a high-performance, distributed, cloud-native time-series database under Apache incubation. It offers efficient write, compression and aggregation for time-series data with Prometheus compatibility. Docker images available for IoT, monitoring and alerting scenarios.
...
> `#Time-Series Database` `#Rust` `#Cloud-Native` `#Distributed Database` `#database` `#distributed-database` `#rust` `#rust-lang` `#sql` `#time-series` `#time-series-database` `#timeseries` 
> CnosDB is a cloud-native open-source distributed time-series database built with Rust, designed for IoT, industrial internet, and connected vehicles. It features storage-computing separation, horizontal scaling, multi-tenancy with role-based access control, and supports SQL queries with schema-less writing for high performance and compression.
...
> `#Dynamic Form` `#Reactive Programming` `#R2DBC` `#Java ORM` `#easyorm` `#javaorm` `#orm` `#sql-generation` 
> A lightweight Java ORM framework designed for dynamic form scenarios. Supports dynamic table structure maintenance and CRUD operations with frontend-passable dynamic query conditions and built-in SQL injection prevention. Offers extensible generic condition queries beyond traditional operators. Native reactive programming support via R2DBC wrapper, seamlessly integrated with Reactor for true non-blocking data access.
...
> `#Distributed Transactions` `#RPC Framework` `#Microservices` `#SpringBoot` `#atomikos` `#consul` `#dubbo` `#dubbo-http` `#grpc` `#jedis` `#nacos` `#ribbon` `#rpc` `#seata` `#sharding-jdbc` `#springboot` `#springcloud` `#tcc` `#thrift` 
> A comprehensive Java SpringBoot integration repository featuring distributed transaction solutions (Atomikos, Seata TCC/AT), RPC frameworks (gRPC, Thrift, Dubbo), database middleware (ShardingSphere), service discovery (Nacos, Consul), Redis clients (Jedis, Lettuce, Redisson), and SpringCloud ecosystem examples—an essential resource for developers mastering microservices architecture and middleware integration.
...
> `#Windows Subsystem` `#Android Compatibility` `#System Backport` `#WSA` 
> This is an open-source backport project that enables Windows Subsystem for Android (WSA) from Windows 11 to run on Windows 10. By modifying WSAClient.exe and WSApatch.dll components, users can run Android applications on Windows 10. The project is archived but provides complete installation scripts and detailed setup guides.
...
> `#Hot Reloading` `#Performance Profiling` `#Python Development` `#Django/Flask Integration` `#ai` `#artificial-intelligence` `#chatgpt` `#django` `#edit-and-continue` `#flask` `#hot-reload` `#hot-reloading` `#pandas` `#python` `#python3` 
> Reloadium is a Python hot reloading and profiling tool enabling instant code updates on save. It provides frame-level reloading, multi-file dependency tracking, and database rollback, with ChatGPT integration for AI-assisted development, significantly improving Python development workflow.
...
> `#File Compression` `#Zstandard Compression` `#Multi-format Archive` `#7-Zip Fork` `#7-zip` `#7z` `#7zip` `#archiver` `#brotli` `#flzma2` `#lizard` `#lz4` `#lz5` `#lzip` `#lzma` `#lzma2` `#zstandard` `#zstd` 
> 7-Zip ZS is an enhanced version of 7-Zip with native support for six modern compression algorithms: Zstandard, Brotli, LZ4, LZ5, Lizard, and Fast-LZMA2. These algorithms offer a wide range from ultra-fast to high-compression options for various use cases. The project provides both full installer (with GUI and Explorer integration) and standalone codec plugin.
...
> [!info]
> ⭐ 54,349 · Jupyter Notebook · 2024-09-18  
...
> [!info]
> ⭐ 5,857 · SCSS · 2026-04-26  
> [GitHub](https://github.com/Keldos-Li/typora-latex-theme)  
> `#Typora Theme` `#LaTeX Typography` `#Academic Writing` `#CSS Theme` `#latex` `#typora` `#typora-theme` `#typora-themes` 
> A Typora theme mimicking Chinese LaTeX document style, designed for undergraduate course papers and lab reports. Default template follows Zhejiang University format with light/dark modes. Customizable via CSS for fonts and margins. Write in Markdown, export to PDF for professional LaTeX-like typography while maintaining simplicity.
...
> `#WSL 2` `#systemd` `#Multi-distro Installer` `#Windows Integration` `#almalinux` `#amazonlinux` `#arch-linux` `#centos` `#debian` `#fedora` `#gentoo` `#linuxmint` `#oraclelinux` `#rockylinux` `#ubuntu` `#wsl` `#wsl-environment` `#wsl-installation` `#wsl2` 
> Distrod is a systemd-based meta-distro for WSL 2 that enables installing Ubuntu, Arch, Debian, Gentoo and more with systemd in under a minute. It provides auto-start on Windows boot and built-in port forwarding to expose Linux services outside Windows.
...
> `#Spring Cloud 2025` `#SaaS Multi-tenant` `#Microservices` `#Enterprise Platform` 
> SpringBlade is a commercial-grade microservice architecture platform supporting both SpringCloud distributed and SpringBoot monolithic deployment. Built with Java17, Spring Boot 3.5, Spring Cloud 2025, it provides SaaS multi-tenant capabilities, multi-terminal authentication, and JWT security. Integrated with Nacos and Sentinel, featuring dual frontend frameworks (React/Vue). Stable in production for 6 years, now at V4.8.0.
...
> [!info]
> ⭐ 7,132 · Python · 2026-06-15  
> [GitHub](https://github.com/vladmandic/sdnext) · [Website](https://vladmandic.github.io/sdnext/)  
> `#Diffusion Models` `#WebUI` `#Multi-Platform Support` `#AI Image Generation` `#ai-art` `#caption` `#diffusers` `#generative-art` `#python` `#pytorch` `#sdnext` `#stable-diffusion` `#transformers` `#webui` 
...
> `#Kubernetes` `#Container Platform` `#Multi-cloud` `#DevOps` `#agent-skills` `#ai` `#cloud-native` `#cncf` `#devops` `#ebpf` `#hacktoberfest` `#kubernetes` `#kubesphere` `#llm` `#multi-cluster` `#multi-tenancy` `#observability` `#servicemesh` `#skills` `#skills-sh` `#skillsmp` 
> KubeSphere is a distributed operating system for cloud-native application management built on Kubernetes, providing unified container management across multi-cloud, data centers, and edge environments. With a plug-and-play extensible architecture, it offers multi-tenant management, DevOps pipelines, application store, and cluster operations through an intuitive web UI.
...
> `#Markdown Editor` `#Typora` `#Legacy Version` `#Installer` `#free` `#typora` 
> Repository hosting Typora v0.11.18 installer. Typora is a cross-platform Markdown editor, and v0.11.18 was the last free version before the software transitioned to a paid model.
...
> `#AI Agent Platform` `#Low-code Automation` `#Workflow Orchestration` `#Docker Containerization` `#agentic-ai` `#agents` `#ai` `#artificial-intelligence` `#autonomous-agents` `#claude` `#gpt` `#llama-api` `#llm` `#openai` `#python` 
...
> `#AutoJS` `#Ant Forest` `#Android Automation` `#Image Recognition` `#autojs` `#javascript` 
> An AutoJS-based automation script for Alipay's Ant Forest that automatically collects energy from friends. Features smart countdown detection, multi-account switching, automatic screen unlock, and visual configuration. Runs on Android 7.0+ without ROOT support.
...
> `#Auto.js` `#Game Automation` `#OCR Recognition` `#Gray Raven` 
> An Auto.js-based game assistant script for "Punishing: Gray Raven" providing automation, OCR text recognition, image recognition, and coordinate management with multilingual UI, floaty window, and runtime logs, requiring Auto.js Pro or Autox.js environment.
...
> `#PostgreSQL HA` `#Cloud-Native Database` `#Streaming Replication` `#Kubernetes Integration` `#cloud-native` `#consul` `#data-consistency` `#declarative-config` `#disaster-recovery` `#docker` `#etcd` `#golang` `#high-availability` `#kubernetes` `#postgresql` `#standby-cluster` `#synchronous-replication` 
> Stolon is a cloud native PostgreSQL HA manager using streaming replication for automatic failover. Its architecture comprises Keeper for instance management, Sentinel for cluster view computation, and Proxy for client access, supporting Kubernetes, etcd, and consul backends with features including service discovery, dynamic reconfiguration, PITR, and standby clusters.
...
> `#Bookmarklet` `#ChatGPT Enhancement` `#Session Management` `#AI-Generated Code` `#cgpt` `#chatgpt` `#chrome-bookmark` `#chrome-bookmark-extesnsion` `#chrome-plugin` `#gpt` `#gpt-turbo` `#tampermonkey` `#tool` `#toolbox` `#userscript` 
> An AI-written ChatGPT enhancement toolkit running as a browser bookmarklet. Features include bypassing high-load login restrictions, disabling data monitoring, connection maintenance, mixed API access, session import/export, chat history download, and GPT4-Mobile unlocking. No plugin installation required—just add the JS code to bookmarks for immediate use.
...
> [!info]
> ⭐ 17,227 · Java · 2025-07-01  
> [GitHub](https://github.com/alibaba/DataX)  
> `#Data Synchronization` `#ETL` `#Plugin Architecture` `#Heterogeneous Data Sources` 
> DataX is Alibaba's open-source offline data synchronization framework, serving as the core engine for阿里云DataWorks data integration and widely used within Alibaba. It adopts a plugin-based architecture, enabling efficient data transfer between 50+ heterogeneous data sources including MySQL, Oracle, HDFS, Hive, and MaxCompute through Reader/Writer plugins, supporting full library migration, incremental sync, and sharding scenarios.
...
> `#Xposed Module` `#Music Source Proxy` `#NetEase Cloud Music` `#Android Reverse Engineering` `#android` `#neteasecloudmusic` `#xposed` 
> Dolby Beta is an Xposed module for NetEase Cloud Music that bypasses regional copyright restrictions via source proxy. It enables users to access unavailable tracks and enjoy higher audio quality (up to 320K). Embedded into NetEase Music settings with AndroidX API, it supports the latest app versions.
...
> `#Distributed Transaction` `#Microservices` `#AT Mode` `#XA Protocol` `#at` `#consistency` `#distributed-transaction` `#microservice` `#saga` `#tcc` `#xa` 
> Apache Seata is a distributed transaction solution from Apache Software Foundation, designed for microservices architecture. It supports AT, TCC, Saga, and XA transaction modes with high performance and ease of use. Core components include Transaction Coordinator, Transaction Manager, and Resource Manager, implementing parse-execute in phase one and commit/rollback in phase two to solve cross-service data consistency issues.
...
> `#Python library` `#Windows notification` `#Zero-dependency` `#Desktop application` `#notification` `#notifications` `#python` `#python-library` `#python3` `#windows-10` `#windows-desktop` 
> winotify is a pure Python library for creating Windows 10 toast notifications without external dependencies, using only the built-in PowerShell. It supports stay-in-action-center notifications, clickable notifications with up to 5 action buttons, custom icons and audio, ideal for Windows desktop application notifications.
...
> `#Python Library` `#Windows Toast` `#Notification` `#Callback` `#module` `#notification-center` `#notifications` `#python` `#python3` `#python37` `#pythonmodule` `#windows` `#windows-10` `#windows10` 
> A lightweight Python library for displaying Windows 10/11 Toast notifications. This enhanced fork of win10toast introduces callback_on_click functionality, enabling users to trigger custom functions—such as opening a URL—when clicking notifications. Supports threaded mode for parallel execution.
...
> `#Tag Autocomplete` `#Stable Diffusion` `#Danbooru Tags` `#WebUI Extension` 
> A Booru-style tag autocompletion extension for AUTOMATIC1111's Stable Diffusion WebUI. Supports Danbooru and e621 tags (top 100k), instant completion hints, keyboard navigation, dark/light mode. Also supports Wildcards, Extra Networks (Embeddings, Hypernets, LoRA, LyCORIS), Chants custom presets, and multilingual translations.
...
> `#Geohash Algorithm` `#Geospatial Encoding` `#WGS84 Coordinates` `#Spatial Indexing` 
> Pure Java implementation of the Geohash algorithm supporting 1-12 character encoding (5-60 bits) and full 64-bit long precision. Compatible with geohash.org reference implementation in character mode, providing WGS84 coordinate encoding, bounding box calculations, cross-180° meridian support, and nearest-neighbor hash queries for location-based storage and proximity search.
...
> `#Neural Rendering` `#Attention Mechanism` `#Anime Character Animation` `#U-Net` 
> Live3D v2 is a neural rendering system for anime character animation using attention mechanisms. It employs an enhanced U-Net architecture with self-attention modules, replacing the traditional ResNet50 encoder to enable conversion from 2D images or drawings to dynamic anime characters. Supports video generation, character colorization, and 3D point cloud reconstruction with Windows one-click deployment.
...
> `#Pixiv` `#Python` `#Image Downloader` `#Web UI` `#acg` `#pixiv` `#pixiv-client` `#pixiv-downloader` `#python` 
> Python-based Pixiv assistant with web UI, enabling artwork search with favorites/popularity sorting, original image downloads via single/multithread/aria2, fetching user works/followings/recommendations, and filtering by dimensions, type, and tags.
> <sub>Pixiv 辅助工具</sub>
...
> `#Markdown Editor` `#Electron` `#Cross-platform Desktop` `#Writing Tool` `#dark-mode` `#editor` `#electron` `#focus-mode` `#latex` `#linux` `#mac` `#macos` `#markdown` `#marktext` `#source-code` `#typewriter-mode` `#vue` `#windows` 
...
> `#System Refactoring` `#Legacy Systems` `#DDD` `#Technical Debt` `#agile` `#ddd` `#ebook` `#guard` `#legacy-system` `#migration` `#refactoring` `#tdd` `#testing` 
> Comprehensive guide for system refactoring and migration, covering legacy system analysis, refactoring strategy development, four-level refactoring patterns (architecture/service/module/code), database refactoring, and architecture governance. Uses EPDCA cycle and DDD methodology with technical debt assessment and automated testing protection plans.
...
> `#Code Generation` `#ORM Framework` `#Fluent API` `#Multi-database Support` 
> Fluent-Mybatis enhances MyBatis with code generation and fluent API, streamlining database operations across multiple databases (MySQL, Oracle, PostgreSQL, etc.). It generates Entity classes, Mapper interfaces and Service layers automatically, providing rich built-in CRUD methods and condition builders without XML configuration.
...
> `#Domain-Driven Design` `#JPA Persistence` `#E-commerce` `#Java EE` 
> A practical sample demonstrating DDD (Domain-Driven Design) persistence with JPA, using a Tmall-like e-commerce business scenario. It showcases core DDD concepts including Entities, Value Objects, and Aggregate Roots implemented with JPA, covering e-commerce domains like orders, products, and users.
...
> `#Cloud Storage Abstraction` `#Spring Boot Integration` `#Multi-Cloud Adapter` `#Distributed File Storage` `#bos` `#file-storage` `#file-upload` `#ftp` `#java` `#kodo` `#minio` `#oss` `#s3` `#sftp` `#spring` `#uss` `#webdav` 
> One-line code to store files across 30+ cloud storage platforms including Aliyun OSS, Tencent Cloud COS, AWS S3, MinIO, and FastDFS. Provides unified API for file upload, download, migration, resumable upload and pre-signed URLs, designed for Java/Spring Boot applications.
...
> `#Spring Data JPA` `#Dynamic Query` `#Specification` `#Builder Pattern` `#dynamic-queries` `#hibernate` `#jpa` `#spring-data-jpa` 
> A dynamic query framework built on Spring Data JPA, inspired by Hibernate Criteria API. Provides builder-style chain APIs for constructing complex queries with conditions like Equal, NotEqual, Like, In, and NotIn. Supports left joins, pagination, sorting, and custom specifications, ideal for Java enterprise applications requiring flexible dynamic queries.
...
> `#Spring Data JPA` `#Dynamic EntityGraph` `#JPA Extension` `#Runtime Optimization` `#entity-graph` `#jpa` `#orm` `#spring-data` `#spring-data-jpa` 
> This library extends Spring Data JPA to enable dynamic EntityGraph usage at runtime. Unlike Spring Data JPA's static annotation-based EntityGraph, it allows passing EntityGraph as a method parameter, letting developers choose which related entities to load dynamically per request, optimizing queries for different use cases.
...
> `#Chinese Name Generator` `#Java Library` `#Random Name Tool` `#Surname Database` 
> KCNamer is a random Chinese name generator that creates names by combining surnames from a built-in database with ~1000 modern Chinese given names, or using completely random characters. It offers gender, length, and surname customization plus DIY name library support, available as standalone JAR or Java library.
...
> `#Spring MVC` `#Parameter Resolver` `#JSON Parsing` `#Java EE` 
> A Spring framework extension addressing @RequestBody annotation limitations in Spring MVC. Enables parsing multiple entity objects and primitive wrapper types in a single request. Provides flexible parsing strategies via annotation value, parameter name matching, and property auto-mapping. Supports various HTTP methods and allows extra JSON properties without errors through custom parameter resolvers.
...
> `#Vue3` `#Element Plus` `#TypeScript` `#Admin System` `#element-plus` `#pinia` `#typescript` `#vue` 
> A comprehensive backend admin system template built with Vue3, Pinia, Element Plus, and TypeScript, powered by Vite 3. Features include authentication, dashboard, data tables/forms, charts, rich text/Markdown editor, image upload, permission management, three-level menus, custom icons, and theme switching. With clean code structure, ideal for rapid development of outsourcing projects or enterprise internal management systems.
...
> `#Rust` `#bdex Protocol` `#Chunk Download` `#CLI Tool` 
> A pure Rust CLI tool for downloading files via bdex:// protocol. Supports multi-threaded parallel downloads, configurable retry attempts, and hash verification skipping. It downloads blocks to a temporary directory in parallel, merges them into a complete file, then automatically cleans up. Ideal for high-speed large file downloads.
...
> `#Smart Parking` `#Vue3 Mobile` `#Vant UI` `#Charging Management` 
> A smart parking and EV charging integrated mobile application built with Vue3 and Vant UI component library. It provides parking navigation, space reservation, and online charging payment, combining parking and charging scenarios for a seamless user experience.
...
> `#Vuepress` `#Java Learning` `#Static Blog` `#GitHub Pages` `#java` `#markdown` `#vuepress` 
> A Vuepress-powered Java learning technical notes blog deployed on GitHub Pages. Covering mainstream Java learning paths like Atguigu and Kuangshen, with detailed code learning records and deployment guides.
> <sub>学习代码的一些笔记</sub>
...
> `#Network Bypass` `#Windows Desktop App` `#Network Restriction Circumvention` 
> EShareTool is a Windows desktop application designed to bypass network sharing restrictions imposed by ESurfingClient (China Telecom's campus network client). It enables multiple devices to connect simultaneously by circumventing client-side network blocks, supporting Windows 7 and above with no VC++ runtime required.
...
> `#PRoot` `#Termux` `#Android Linux` `#Linux Distribution` `#linux` `#termux` 
> A one-click script for installing Linux distributions (Ubuntu, Kali, Debian) on Termux terminal emulator. Uses domestic Gitee mirrors for faster downloads and pre-configures Chinese software repositories, addressing slow foreign resource access. Runs full Linux on Android via PRoot without root required.
...
> `#Python3` `#UmaMusume` `#Simulator Config` `#Game Data` `#umacruise` `#umamusume` 
> A Python3-based config builder for the UmaCruise-U racing game simulator, generating database configurations from the UraraWin data source to help players quickly set up the game environment.
...
> `#Game Assistant` `#Computer Vision` `#OCR` `#Windows Desktop` 
> Automation tool for DMM Uma Musume using OpenCV and Tesseract OCR to detect game windows, recognize event choices, and display their effects. Features URA race prediction and race reservation to optimize training strategies.
...
> `#CoreAudioToolbox` `#AAC Encoder` `#ALAC Encoder` `#CLI Tool` 
> qaac is a CLI audio encoder leveraging Apple CoreAudioToolbox to encode AAC and ALAC formats. It requires iTunes installation for CoreAudioToolbox DLLs. Note: CoreAudioToolbox versions 7.9.8.x through 7.10.9.0 have glitch issues in AAC CBR mode.
...
> `#Python Automation` `#Epidemic Reporting` `#Campus Cloud` `#Scheduled Task` 
> Python script for automatic daily epidemic report submission on 今日校园 (Campus Cloud). Supports email notifications and works via cloud system login or universal API. Compatible with most universities and deployable on Tencent Cloud Function or self-hosted servers for scheduled execution.
...
> `#C++ Debug Macro` `#Type Inspection` `#Single-header Library` `#Expression Evaluation` `#cpp` `#debugging` `#macro` `#pretty-printing` 
> A single-header C++ debugging macro library providing dbg(...) as a printf replacement. Features colorized terminal output, prints file/line/function names with type info, pretty-prints containers/smart pointers, works in expressions, and warns on inclusion. Compatible with C++11/14/17.
...
> `#Spring Boot` `#RBAC Authorization` `#Code Generator` `#JPA ORM` `#admin` `#eladmin` `#element-ui` `#jpa` `#jwt` `#redis` `#spring-boot` `#spring-security` `#swagger2` `#vue` 
> A Spring Boot 2.7.18 and Jpa-based full-stack admin system with modular architecture. Features RBAC permission control via Spring Security and Redis, supporting data dictionaries, data permissions, dynamic routing, and one-click code generation. Includes enterprise features like logging, scheduled tasks, and DevOps deployment with Vue frontend.
